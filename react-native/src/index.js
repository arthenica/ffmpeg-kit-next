...
/**
 * <p>An in-memory input that exposes a byte array to <code>FFmpeg</code> through the
 * <code>ffkit</code> protocol. Pass the value returned by {@link #getUrl} as an input of an
 * <code>FFmpeg</code> command instead of a file path. Call {@link #close} when the buffer is no
 * longer needed to release the native resources it holds.
 *
 * <p>Buffer data is exchanged with the native layer as base64 encoded strings.
 */
export class FFmpegKitInputBuffer {
  ...
  /**
   * Registers base64 encoded data as an input buffer and returns the wrapper that holds the
   * generated protocol url.
   *
   * @param base64Data base64 encoded buffer data
   * @param extension optional hint (e.g. "mp4") that helps FFmpeg detect the input format
   * @return a new input buffer instance
   */
  static async fromBase64(base64Data, extension) {
    await FFmpegKitConfig.init();

    const url = await FFmpegKitReactNativeModule.inputBufferFromByteArray(base64Data, extension ?? null);
    return new FFmpegKitInputBuffer(url ?? "", FFmpegKitFactory.base64ByteLength(base64Data));
  }
   ...
   */
  async close() {
    return FFmpegKitReactNativeModule.inputBufferClose(this.#url);
  }

}

/**
 * <p>An in-memory output that collects <code>FFmpeg</code> output through the <code>ffkit</code>
 * protocol. Pass the value returned by {@link #getUrl} as an output of an <code>FFmpeg</code>
 * command instead of a file path, then read the produced bytes with {@link #toBase64} once the
 * command completes. Call {@link #close} to release the native resources it holds.
 *
 * <p>Buffer data is exchanged with the native layer as base64 encoded strings.
 */
export class FFmpegKitOutputBuffer {
  ...
  /**
   * Creates an output buffer and returns the wrapper that holds the generated protocol url.
   *
   * @param extension optional format hint (e.g. "mp4")
   * @param initialCapacity optional initial native buffer size in bytes
   * @param maxCapacity optional maximum native buffer size in bytes; must be provided together
   * with <code>initialCapacity</code>
   * @return a new output buffer instance
   */
  static async create(extension, initialCapacity, maxCapacity) {
    await FFmpegKitConfig.init();

    const url = await FFmpegKitReactNativeModule.outputBufferCreate(extension ?? null, initialCapacity ?? null, maxCapacity ?? null);
    return new FFmpegKitOutputBuffer(url ?? "");
  }
   ...
   */
  async getSize() {
    return FFmpegKitReactNativeModule.outputBufferGetSize(this.#url);
  }

  /**
   * Returns the bytes written by FFmpeg as a base64 encoded string.
   *
   * @return base64 encoded buffer data
   */
  async toBase64() {
    return FFmpegKitReactNativeModule.outputBufferToByteArray(this.#url);
  }
   ...
   */
  async close() {
    return FFmpegKitReactNativeModule.outputBufferClose(this.#url);
  }

}

/**
 * <p>A streaming input that feeds bytes to <code>FFmpeg</code> incrementally through the
 * <code>ffkit</code> protocol. Pass the value returned by {@link #getUrl} as an input of an
 * <code>FFmpeg</code> command, push data with {@link #write} while the command runs, then signal
 * the end of the input with {@link #closeInput}. Call {@link #close} to release the native
 * resources it holds.
 *
 * <p>Buffer data is exchanged with the native layer as base64 encoded strings.
 */
export class FFmpegKitStreamInput {
  ...
  /**
   * Creates a stream input and returns the wrapper that holds the generated protocol url.
   *
   * @param extension optional format hint (e.g. "mp4")
   * @param capacity optional native ring-buffer capacity in bytes
   * @return a new stream input instance
   */
  static async create(extension, capacity) {
    await FFmpegKitConfig.init();

    const url = await FFmpegKitReactNativeModule.streamInputCreate(extension ?? null, capacity ?? null);
    return new FFmpegKitStreamInput(url ?? "");
  }
  ...
  /**
   * Writes base64 encoded data to the stream, blocking up to <code>timeoutMs</code> milliseconds
   * when the native buffer is full.
   *
   * @param base64Data base64 encoded data to write
   * @param timeoutMs optional write timeout in milliseconds
   * @return number of bytes accepted
   */
  async write(base64Data, timeoutMs) {
    return FFmpegKitReactNativeModule.streamInputWrite(this.#url, base64Data, timeoutMs ?? null);
  }

  /**
   * Signals that no more data will be written, letting FFmpeg reach end-of-input.
   */
  async closeInput() {
    return FFmpegKitReactNativeModule.streamInputCloseInput(this.#url);
  }
   ...
   */
  async close() {
    return FFmpegKitReactNativeModule.streamInputClose(this.#url);
  }

}

/**
 * <p>A streaming output that pulls bytes from <code>FFmpeg</code> incrementally through the
 * <code>ffkit</code> protocol. Pass the value returned by {@link #getUrl} as an output of an
 * <code>FFmpeg</code> command, then consume the produced data with {@link #read} while the command
 * runs. Call {@link #close} to release the native resources it holds.
 *
 * <p>Buffer data is exchanged with the native layer as base64 encoded strings.
 */
export class FFmpegKitStreamOutput {
  ...
  /**
   * Creates a stream output and returns the wrapper that holds the generated protocol url.
   *
   * @param extension optional format hint (e.g. "mp4")
   * @param capacity optional native ring-buffer capacity in bytes
   * @return a new stream output instance
   */
  static async create(extension, capacity) {
    await FFmpegKitConfig.init();

    const url = await FFmpegKitReactNativeModule.streamOutputCreate(extension ?? null, capacity ?? null);
    return new FFmpegKitStreamOutput(url ?? "");
  }
  ...
  /**
   * Reads up to <code>maxBytes</code> bytes produced by FFmpeg, blocking up to
   * <code>timeoutMs</code> milliseconds when no data is available yet.
   *
   * @param maxBytes maximum number of bytes to read
   * @param timeoutMs optional read timeout in milliseconds
   * @return base64 encoded data, or an empty string when the output has ended
   */
  async read(maxBytes, timeoutMs) {
    const data = await FFmpegKitReactNativeModule.streamOutputRead(this.#url, maxBytes, timeoutMs ?? null);
    return data ?? "";
  }
   ...
   */
  async close() {
    return FFmpegKitReactNativeModule.streamOutputClose(this.#url);
  }
...
class FFmpegKitFactory {
  ...
  static base64ByteLength(base64Data) {
    if (!base64Data) {
      return 0;
    }
    const length = base64Data.length;
    let padding = 0;
    if (base64Data.charAt(length - 1) === '=') {
      padding++;
    }
    if (base64Data.charAt(length - 2) === '=') {
      padding++;
    }
