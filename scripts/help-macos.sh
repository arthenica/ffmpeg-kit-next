...
enabled without any external libraries. Options can be used to select architectures and/or enable external libraries. \
...
When compilation ends, libraries are created under the prebuilt folder.\n"
  ...
  echo -e "  --enable-lib-macos-audiotoolbox\tbuild with built-in Apple AudioToolbox support [no]"
  echo -e "  --enable-lib-macos-avfoundation\tbuild with built-in Apple AVFoundation support [no]"
  echo -e "  --enable-lib-macos-bzip2\tbuild with built-in bzip2 support [no]"
  echo -e "  --enable-lib-macos-videotoolbox\tbuild with built-in Apple VideoToolbox support [no]"
  echo -e "  --enable-lib-macos-zlib\tbuild with built-in zlib [no]"
  echo -e "  --enable-lib-macos-libiconv\tbuild with built-in libiconv [no]"
  echo -e "  --enable-lib-macos-coreimage\tbuild with built-in Apple CoreImage support [no]"
  echo -e "  --enable-lib-macos-opencl\tbuild with built-in OpenCL [no]"
  echo -e "  --enable-lib-macos-opengl\tbuild with built-in OpenGL [no]"
