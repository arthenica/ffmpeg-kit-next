...
# CHECK IF OUR WORKAROUND IS IN PLACE, IF NOT, RECONFIGURE
if [[ -f "${BASEDIR}"/src/"${LIB_NAME}"/src/gl/time.in.h ]] && ! grep -q "__ANDROID_API__ < 35" "${BASEDIR}"/src/"${LIB_NAME}"/src/gl/time.in.h; then
  export RECONF_gnutls=1
fi

# REGENERATE BUILD FILES IF NECESSARY OR REQUESTED
if [[ ! -f "${BASEDIR}"/src/"${LIB_NAME}"/configure ]] || [[ ${RECONF_gnutls} -eq 1 ]]; then
  git submodule update --init gnulib || return 1
  ./bootstrap --skip-po --no-git --gnulib-srcdir=gnulib || return 1
  # PATCH TIMEZONE_T USAGE TO AVOID BUILD FAILURES REGARDING TIMEZONE_T DEFINITIONS
  perl -0pi -e 's/# if defined _GNU_SOURCE && \@GNULIB_TIME_RZ\@ && ! \@HAVE_TIMEZONE_T\@/# if defined _GNU_SOURCE \&\& \@GNULIB_TIME_RZ\@ \&\& (! \@HAVE_TIMEZONE_T\@ || (defined __ANDROID__ \&\& __ANDROID_API__ < 35))/g; s/typedef struct tm_zone \*timezone_t;/#  if \@HAVE_TIMEZONE_T\@ \&\& defined __ANDROID__\ntypedef struct tm_zone *rpl_timezone_t;\n#   define timezone_t rpl_timezone_t\n#  else\ntypedef struct tm_zone *timezone_t;\n#  endif/g' src/gl/time.in.h || return 1
