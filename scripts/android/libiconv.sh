...
# gnulib-tool's module cache can misdetect associative-array support under
# some Linux /bin/sh implementations and then treats module names containing
# slashes as arithmetic expressions. Disable the cache for libiconv autogen.
${SED_INLINE} 's|GNULIB_TOOL="$GNULIB_TOOL"|GNULIB_TOOL="$GNULIB_TOOL --no-cache-modules"|g' "${BASEDIR}"/src/"${LIB_NAME}"/autogen.sh || return 1
