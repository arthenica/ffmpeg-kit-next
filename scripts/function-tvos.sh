...
get_common_cflags() {
  ...
  case ${ARCH} in
  arm64)
    echo "-fstrict-aliasing ${BITCODE_FLAGS} -DTVOS $(get_min_sdk_version_flag) ${BUILD_DATE} ${USES_FFMPEG_KIT_PROTOCOLS} -Wno-incompatible-function-pointer-types -isysroot ${SDK_PATH}"
    ;;
  x86-64 | arm64-simulator)
    echo "-fstrict-aliasing -DTVOS $(get_min_sdk_version_flag) ${BUILD_DATE} ${USES_FFMPEG_KIT_PROTOCOLS} -Wno-incompatible-function-pointer-types -isysroot ${SDK_PATH}"
    ;;
  esac
}
