...
get_ffmpeg_kit_version() {
  local FFMPEG_KIT_VERSION=$(sed -n 's/.*const FFmpegKitVersion.*@"\([^"]*\)".*/\1/p' "${BASEDIR}"/apple/src/FFmpegKitConfig.m)
...
enable_ios_main_build() {
  ...
  if [[ $(compare_versions "$DETECTED_IOS_SDK_VERSION" "14.0") -le 0 ]]; then
    export MAC_CATALYST_MIN_VERSION=13.1
  else
...
get_framework_directory() {
  ...
  "${ARCH_VAR_IOS}")
    echo "bundle-apple-framework-ios-${IOS_MIN_VERSION}"
    ;;
  "${ARCH_VAR_IPHONEOS}")
    echo "bundle-apple-framework-iphoneos-${IOS_MIN_VERSION}"
    ;;
  "${ARCH_VAR_IPHONESIMULATOR}")
    echo "bundle-apple-framework-iphonesimulator-${IOS_MIN_VERSION}"
    ;;
  "${ARCH_VAR_MAC_CATALYST}")
    echo "bundle-apple-framework-mac-catalyst-${MAC_CATALYST_MIN_VERSION}"
    ;;
  "${ARCH_VAR_TVOS}")
    echo "bundle-apple-framework-tvos-${TVOS_MIN_VERSION}"
    ;;
  "${ARCH_VAR_APPLETVOS}")
    echo "bundle-apple-framework-appletvos-${TVOS_MIN_VERSION}"
    ;;
  "${ARCH_VAR_APPLETVSIMULATOR}")
    echo "bundle-apple-framework-appletvsimulator-${TVOS_MIN_VERSION}"
    ;;
  "${ARCH_VAR_MACOS}")
...
framework module ffmpegkit {
  ...
  header "FFmpegKitInputBuffer.h"
  header "FFmpegKitOutputBuffer.h"
  header "FFmpegSession.h"
  header "FFmpegKitStreamInput.h"
  header "FFmpegKitStreamOutput.h"
...
#
# Creates a local Swift package manifest (Package.swift) inside the given
# xcframework bundle directory ($1) so the prebuilt bundle can be consumed
# directly via SPM, e.g.
#
#   .package(path: "<path to the prebuilt xcframework bundle folder>")
#   import ffmpegkit
#
# Used for both the apple umbrella bundle and the per-platform (ios/macos/tvos)
# xcframework bundles. The manifest references the xcframeworks by relative
# path, therefore it does not require hosting, versioning or checksums. The
# framework set is identical across build variants (min, https, full, ...);
# only the binary content differs, exactly as with the CocoaPods packages.
#
create_spm_package() {
  local PACKAGE_DIRECTORY="$1"
  local PACKAGE_FILE="${PACKAGE_DIRECTORY}/Package.swift"

  # 1. BINARY TARGETS AND PRODUCT TARGET REFERENCES (FFMPEG LIBRARIES + FFMPEGKIT)
  local BINARY_TARGETS=""
  local PRODUCT_TARGETS=""
  for LIBRARY in "${FFMPEG_LIBS[@]}" "ffmpegkit"; do
    BINARY_TARGETS+="        .binaryTarget(name: \"${LIBRARY}\", path: \"${LIBRARY}.xcframework\"),"$'\n'
    PRODUCT_TARGETS+="                \"${LIBRARY}\","$'\n'
  done

  # 2. SUPPORTED PLATFORMS
  #    per-platform builds expose a single platform; the apple umbrella build
  #    derives the platforms from the architecture variants packaged
  local PLATFORMS=""
  case ${FFMPEG_KIT_BUILD_TYPE} in
  ios)
    PLATFORMS+="        .iOS(\"${IOS_MIN_VERSION:-12.1}\"),"$'\n'
    ;;
  macos)
    PLATFORMS+="        .macOS(\"${MACOS_MIN_VERSION:-10.15}\"),"$'\n'
    ;;
  tvos)
  ...
  *)
    ...
    for VARIANT in "${TARGET_ARCHITECTURE_VARIANT_INDEX_ARRAY[@]}"; do
      ...
      "${ARCH_VAR_IPHONEOS}" | "${ARCH_VAR_IPHONESIMULATOR}" | "${ARCH_VAR_MAC_CATALYST}")
        HAS_IOS=1
        ;;
      "${ARCH_VAR_APPLETVOS}" | "${ARCH_VAR_APPLETVSIMULATOR}")
        HAS_TVOS=1
        ;;
      "${ARCH_VAR_MACOS}")
        HAS_MACOS=1
...
EOF
  ...
  echo -e "DEBUG: Created local Swift package manifest at ${PACKAGE_FILE}\n" 1>>"${BASEDIR}"/build.log 2>&1
}
...
create_zlib_system_package_config() {
  ZLIB_VERSION=$(sed -n 's/.*#define ZLIB_VERSION "\([^"]*\)".*/\1/p' "${SDK_PATH}"/usr/include/zlib.h)
