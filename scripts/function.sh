...
get_arch_name() {
  ...
  5) echo "arm64" ;; # ios, linux, tvos, macos, visionos
  6) echo "arm64e" ;; # ios
  7) echo "i386" ;; # ios
  8) echo "x86" ;; # android
...
from_arch_name() {
  ...
  arm64) echo 5 ;; # ios, linux, tvos, macos, visionos
  arm64e) echo 6 ;; # ios
  i386) echo 7 ;; # ios
  x86) echo 8 ;; # android
...
get_help_command() {
  if [[ -n ${FFMPEG_KIT_HELP_COMMAND:-} ]]; then
    echo "${FFMPEG_KIT_HELP_COMMAND}"
  else
    echo "$0"
  fi
}
...
print_unknown_option() {
  ...
  echo -e "\n(*) Unknown option \"$1\".\n\nSee ${COMMAND} --help for available options.\n"
  exit 1
}
...
print_unknown_library() {
  ...
  echo -e "\n(*) Unknown library \"$1\".\n\nSee ${COMMAND} --help for available libraries.\n"
  exit 1
}
...
print_unknown_arch() {
  ...
  echo -e "\n(*) Unknown architecture \"$1\".\n\nSee ${COMMAND} --help for available architectures.\n"
  exit 1
}
...
print_unknown_arch_variant() {
  ...
  echo -e "\n(*) Unknown architecture variant \"$1\".\n\nSee ${COMMAND} --help for available architecture variants.\n"
  exit 1
}

display_version() {
  COMMAND=$(get_help_command | sed -e 's/\.\///g')
...
set_library() {
  ...
  linux-libass)
    ...
    set_library "linux-harfbuzz" $2
    set_library "linux-libiconv" $2
    ;;
  linux-harfbuzz)
    ENABLED_LIBRARIES[LIBRARY_LINUX_HARFBUZZ]=$2
    set_library "linux-freetype" $2
    ;;
  linux-libiconv)
