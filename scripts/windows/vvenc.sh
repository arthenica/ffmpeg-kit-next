...
if [[ -f "${LIB_INSTALL_PREFIX}/lib/pkgconfig/libvvenc.pc" ]]; then
  copy_and_update_path "${LIB_INSTALL_PREFIX}/lib/pkgconfig/libvvenc.pc" "${INSTALL_PKG_CONFIG_DIR}" || return 1
elif [[ -f "${LIB_INSTALL_PREFIX}/lib64/pkgconfig/libvvenc.pc" ]]; then
  copy_and_update_path "${LIB_INSTALL_PREFIX}/lib64/pkgconfig/libvvenc.pc" "${INSTALL_PKG_CONFIG_DIR}" || return 1
else
  return 1
fi
