...
while [ ${#enabled_library_list[@]} -gt $completed ]; do
  for library in "${enabled_library_list[@]}"; do
    ...
    case $library in
    fontconfig)
      if [[ $OK_expat -eq 1 ]] && [[ $OK_libiconv -eq 1 ]] && [[ $OK_freetype -eq 1 ]]; then
        run=1
      fi
      ;;
    freetype)
      if [[ $OK_zlib -eq 1 ]] && [[ $OK_libpng -eq 1 ]] && [[ $OK_libjxl -eq 1 ]]; then
        run=1
      fi
      ;;
    gnutls)
      if [[ $OK_zlib -eq 1 ]] && [[ $OK_nettle -eq 1 ]] && [[ $OK_gmp -eq 1 ]] && [[ $OK_libiconv -eq 1 ]]; then
        run=1
      fi
      ;;
    harfbuzz)
      ...
      ;;
    leptonica)
      if [[ $OK_zlib -eq 1 ]] && [[ $OK_giflib -eq 1 ]] && [[ $OK_jpeg -eq 1 ]] && [[ $OK_libpng -eq 1 ]] && [[ $OK_tiff -eq 1 ]] && [[ $OK_libwebp -eq 1 ]]; then
        run=1
      fi
      ;;
    libass)
      if [[ $OK_expat -eq 1 ]] && [[ $OK_libiconv -eq 1 ]] && [[ $OK_freetype -eq 1 ]] && [[ $OK_fribidi -eq 1 ]] && [[ $OK_fontconfig -eq 1 ]] && [[ $OK_libpng -eq 1 ]] && [[ $OK_harfbuzz -eq 1 ]]; then
        run=1
      fi
      ;;
    libtheora)
    ...
    libvorbis)
      ...
      ;;
    libxml2)
      if [[ $OK_zlib -eq 1 ]] && [[ $OK_libiconv -eq 1 ]]; then
        run=1
      fi
      ;;
    libpng)
      if [[ $OK_zlib -eq 1 ]]; then
        run=1
      fi
      ;;
    nettle)
    ...
    srt)
      ...
      ;;
    tesseract)
      if [[ $OK_zlib -eq 1 ]] && [[ $OK_leptonica -eq 1 ]]; then
        run=1
      fi
      ;;
    tiff)
      if [[ $OK_zlib -eq 1 ]] && [[ $OK_jpeg -eq 1 ]]; then
        run=1
      fi
      ;;
    twolame)
