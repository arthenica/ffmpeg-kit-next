...
get_common_cflags() {
  ...
  arm64 | arm64-simulator)
    echo "-fstrict-aliasing -DVISIONOS $(get_min_sdk_version_flag) ${BUILD_DATE} ${USES_FFMPEG_KIT_PROTOCOLS} -Wno-incompatible-function-pointer-types -isysroot ${SDK_PATH}"
    ;;
  esac
}
...
get_cxxflags() {
  ...
  case $1 in
  ffmpeg-kit)
    echo "-std=c++11 -fno-exceptions -fno-rtti -fPIC ${COMMON_CFLAGS} ${OPTIMIZATION_FLAGS} $(get_package_name_cflag) ${EXTRA_CXXFLAGS}"
    ;;
  gnutls)
    echo "-std=c++11 -fno-rtti ${COMMON_CFLAGS} ${OPTIMIZATION_FLAGS} ${EXTRA_CXXFLAGS}"
    ;;
  libaom)
    echo "-std=c++11 -fno-exceptions ${COMMON_CFLAGS} ${OPTIMIZATION_FLAGS} ${EXTRA_CXXFLAGS}"
    ;;
  libilbc)
    echo "-std=c++14 -fno-exceptions ${COMMON_CFLAGS} ${OPTIMIZATION_FLAGS} ${EXTRA_CXXFLAGS}"
    ;;
  libwebp | xvidcore)
    echo "-std=c++11 -fno-exceptions -fno-rtti -fno-common -DPIC ${COMMON_CFLAGS} ${OPTIMIZATION_FLAGS} ${EXTRA_CXXFLAGS}"
    ;;
  opencore-amr)
  ...
  libjxl)
    ...
    ;;
  libsvtav1)
    echo "-std=c++11 ${COMMON_CFLAGS} ${OPTIMIZATION_FLAGS} ${EXTRA_CXXFLAGS}"
    ;;
  srt | tesseract | zimg)
    echo "-std=c++11 ${COMMON_CFLAGS} ${OPTIMIZATION_FLAGS} ${EXTRA_CXXFLAGS}"
    ;;
  vvenc)
    echo "-std=c++14 ${COMMON_CFLAGS} ${OPTIMIZATION_FLAGS} ${EXTRA_CXXFLAGS}"
    ;;
  x265)
    echo "-std=c++11 -fno-exceptions ${COMMON_CFLAGS} ${EXTRA_CXXFLAGS}"
    ;;
  *)
    echo "-std=c++11 -fno-exceptions -fno-rtti ${COMMON_CFLAGS} ${OPTIMIZATION_FLAGS} ${EXTRA_CXXFLAGS}"
    ;;
  esac
}
...
get_common_ldflags() {
  echo "-isysroot ${SDK_PATH} $(get_min_version_cflags) -target $(get_target)"
}
...
initialize_prebuilt_visionos_folders() {
  if [[ -n ${FFMPEG_KIT_XCF_BUILD} ]]; then
    ...
    echo -e "DEBUG: Initializing default universal directory at ${BASEDIR}/.tmp/$(get_universal_library_directory "${ARCH_VAR_VISIONOS}")\n" 1>>"${BASEDIR}"/build.log 2>&1

    # DEFAULT BUILDS GENERATE UNIVERSAL LIBRARIES AND FRAMEWORKS
    initialize_folder "${BASEDIR}/.tmp/$(get_universal_library_directory "${ARCH_VAR_VISIONOS}")"

    echo -e "DEBUG: Initializing framework directory at ${BASEDIR}/prebuilt/$(get_framework_directory "${ARCH_VAR_VISIONOS}")\n" 1>>"${BASEDIR}"/build.log 2>&1

    initialize_folder "${BASEDIR}/prebuilt/$(get_framework_directory "${ARCH_VAR_VISIONOS}")"
  fi
}
...
create_universal_libraries_for_visionos_default_frameworks() {
  local ROOT_UNIVERSAL_DIRECTORY_PATH="${BASEDIR}/.tmp/$(get_universal_library_directory "${ARCH_VAR_VISIONOS}")"

  echo -e "INFO: Building universal libraries in ${ROOT_UNIVERSAL_DIRECTORY_PATH} for default frameworks using ${TARGET_ARCH_LIST[@]}\n" 1>>"${BASEDIR}"/build.log 2>&1

  create_ffmpeg_universal_library "${ARCH_VAR_VISIONOS}"

  create_ffmpeg_kit_universal_library "${ARCH_VAR_VISIONOS}"
...
create_visionos_default_frameworks() {
  ...
  create_ffmpeg_framework "${ARCH_VAR_VISIONOS}"

  create_ffmpeg_kit_framework "${ARCH_VAR_VISIONOS}"
