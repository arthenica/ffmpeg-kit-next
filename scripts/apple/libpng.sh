...
# WORKAROUND TO FIX ZLIB PATHS
export CFLAGS="$(pkg-config --cflags zlib) ${CFLAGS}"
export CPPFLAGS="$(pkg-config --cflags zlib) ${CPPFLAGS}"
export LDFLAGS="$(pkg-config --libs zlib) ${LDFLAGS}"
...
# WORKAROUND TO PREVENT SEARCHING FOR FP.H
${SED_INLINE} "s/ || defined(TARGET_OS_MAC)//" "${BASEDIR}/src/${LIB_NAME}/pngpriv.h"
