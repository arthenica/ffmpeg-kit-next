 ...
 * It is derived from FFmpeg's fftools/ffmpeg_sched.c at tag n7.1.5.
 *
...
static int task_start(SchTask *task)
{
    ...
    context = saveFFmpegContext(task);
    if (!context)
        return AVERROR(ENOMEM);

    ret = pthread_create(&task->thread, NULL, task_wrapper, context);
    if (ret) {
...
static void *task_wrapper(void *arg)
{
    FFmpegContext *context = arg;
    SchTask  *task;
    Scheduler *sch;
    int ret;
    int err = 0;

    task = loadFFmpegContext(context);
    av_free(context);
    sch = task->parent;
