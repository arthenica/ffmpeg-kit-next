...
/*
 * This file is the modified version of ffmpeg_demux.c file living in ffmpeg
 ...
 * - global thread local variables migrated to the d->thread via FFmpegContext
 *
 * 07.2023
 * --------------------------------------------------------
...
static const char *const opt_name_fix_sub_duration[] = {"fix_sub_duration",
                                                        NULL};
static const char *const opt_name_canvas_sizes[] = {"canvas_size", NULL};
static const char *const opt_name_guess_layout_max[] = {"guess_layout_max",
                                                        NULL};
...
static const char *const opt_name_display_rotations[] = {"display_rotation",
                                                         NULL};
...
static DemuxStream *ds_from_ist(InputStream *ist) { return (DemuxStream *)ist; }

static Demuxer *demuxer_from_ifile(InputFile *f) { return (Demuxer *)f; }
...
static void report_new_stream(Demuxer *d, const AVPacket *pkt) {
    ...
    av_log(d, AV_LOG_WARNING,
           ...
           pkt->stream_index, pkt->pos,
           av_ts2timestr(pkt->dts, &st->time_base));
...
static void ifile_duration_update(Demuxer *d, DemuxStream *ds,
    ...
    if (!d->duration || av_compare_ts(d->duration, d->time_base, last_duration,
                                      ds->ist.st->time_base) < 0) {
...
static int seek_to_start(Demuxer *d) {
    ...
    ret = avformat_seek_file(is, -1, INT64_MIN, is->start_time, is->start_time,
                             0);
    if (ret < 0)
        return ret;
    ...
    if (ifile->audio_duration_queue_size) {
        ...
        while (got_durations < ifile->audio_duration_queue_size) {
            ...
            ret = av_thread_message_queue_recv(ifile->audio_duration_queue,
                                               &dur, 0);
            if (ret < 0)
                return ret;
            got_durations++;
...
static void ts_discontinuity_detect(Demuxer *d, InputStream *ist,
    ...
    int64_t pkt_dts =
        av_rescale_q_rnd(pkt->dts, pkt->time_base, AV_TIME_BASE_Q,
                         AV_ROUND_NEAR_INF | AV_ROUND_PASS_MINMAX);

    if (copy_ts && ds->next_dts != AV_NOPTS_VALUE && fmt_is_discont &&
        ist->st->pts_wrap_bits < 60) {
        int64_t wrap_dts = av_rescale_q_rnd(
            pkt->dts + (1LL << ist->st->pts_wrap_bits), pkt->time_base,
            AV_TIME_BASE_Q, AV_ROUND_NEAR_INF | AV_ROUND_PASS_MINMAX);
        ...
        if (fmt_is_discont) {
            if (FFABS(delta) > 1LL * dts_delta_threshold * AV_TIME_BASE ||
                ...
                if (pkt->pts != AV_NOPTS_VALUE)
                    pkt->pts -=
                        av_rescale_q(delta, AV_TIME_BASE_Q, pkt->time_base);
            }
        } else {
    ...
    } else if (ds->next_dts == AV_NOPTS_VALUE && !copy_ts && fmt_is_discont &&
...
static void ts_discontinuity_process(Demuxer *d, InputStream *ist,
                                     AVPacket *pkt) {
    int64_t offset =
        av_rescale_q(d->ts_offset_discont, AV_TIME_BASE_Q, pkt->time_base);
...
static int ist_dts_update(DemuxStream *ds, AVPacket *pkt) {
    ...
    if (!ds->saw_first_ts) {
        ds->first_dts = ds->dts = ist->st->avg_frame_rate.num
                                      ? -ist->par->video_delay * AV_TIME_BASE /
    ...
    case AVMEDIA_TYPE_AUDIO:
        ...
        if (par->sample_rate) {
            ds->next_dts +=
                ((int64_t)AV_TIME_BASE * par->frame_size) / par->sample_rate;
        } else {
            ds->next_dts +=
    ...
    case AVMEDIA_TYPE_VIDEO:
        if (ist->framerate.num) {
            ...
            AVRational field_rate =
                av_mul_q(ist->par->framerate, (AVRational){2, 1});
            int fields = 2;
            ...
            ds->next_dts +=
                av_rescale_q(fields, av_inv_q(field_rate), AV_TIME_BASE_Q);
        }
        break;
    }
...
#define SHOW_TS_DEBUG(tag_)                                                    \
    if (debug_ts) {                                                            \
        av_log(ist, AV_LOG_INFO,                                               \
               ...
               av_ts2timestr(pkt->duration, &pkt->time_base));                 \
    }
    ...
    if (pkt->dts != AV_NOPTS_VALUE)
        pkt->dts +=
            av_rescale_q(ifile->ts_offset, AV_TIME_BASE_Q, pkt->time_base);
    if (pkt->pts != AV_NOPTS_VALUE)
        pkt->pts +=
            av_rescale_q(ifile->ts_offset, AV_TIME_BASE_Q, pkt->time_base);
...
static int input_packet_process(Demuxer *d, DemuxMsg *msg, AVPacket *src) {
    ...
    if (debug_ts) {
        av_log(NULL, AV_LOG_INFO,
               "demuxer+ffmpeg -> ist_index:%d:%d type:%s pkt_pts:%s "
...
static void readrate_sleep(Demuxer *d) {
    InputFile *f = &d->f;
    int64_t file_start =
        copy_ts * ((f->start_time_effective != AV_NOPTS_VALUE
                        ? f->start_time_effective * !start_at_zero
                        : 0) +
    ...
    for (int i = 0; i < f->nb_streams; i++) {
        ...
        stream_ts_offset = FFMAX(
            ds->first_dts != AV_NOPTS_VALUE ? ds->first_dts : 0, file_start);
...
static void *input_thread(void *arg) {
    ...
    loadFFmpegContext(context);
    Demuxer *d = (Demuxer *)context->arg;
    av_free(arg);
...
static int thread_start(Demuxer *d) {
    ...
    FFmpegContext *context = saveFFmpegContext();
    context->arg = d;

    if ((ret = pthread_create(&d->thread, NULL, input_thread, context))) {
...
static void demux_final_stats(Demuxer *d) {
    ...
    for (int j = 0; j < f->nb_streams; j++) {
        ...
        av_log(f, AV_LOG_VERBOSE, "  Input stream #%d:%d (%s): ", f->index, j,
               av_get_media_type_string(type));
        av_log(f, AV_LOG_VERBOSE,
               "%" PRIu64 " packets read (%" PRIu64 " bytes); ", ds->nb_packets,
               ds->data_size);
...
static int choose_decoder(const OptionsContext *o, AVFormatContext *s,
                          AVStream *st, enum HWAccelID hwaccel_id,
    ...
    if (codec_name) {
        int ret =
            find_codec(NULL, codec_name, st->codecpar->codec_type, 0, pcodec);
        if (ret < 0)
            return ret;
                ...
                for (int j = 0; (config = avcodec_get_hw_config(c, j)); j++) {
                    if (config->device_type == hwaccel_device_type) {
                        av_log(NULL, AV_LOG_VERBOSE,
                               "Selecting decoder '%s' because of requested "
                               "hwaccel method "
                               "%s\n",
                               c->name,
...
static int add_display_matrix_to_stream(const OptionsContext *o,
    ...
    buf = (int32_t *)sd->data;
    av_display_rotation_set(buf, rotation_set ? -(rotation) : -0.0f);

    av_display_matrix_flip(buf, hflip_set ? hflip : 0, vflip_set ? vflip : 0);
...
static int ist_add(const OptionsContext *o, Demuxer *d, AVStream *st) {
    ...
    if (st->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
        ...
        MATCH_PER_STREAM_OPT(hwaccel_output_formats, str, hwaccel_output_format,
                             ic, st);
    ...
    ret = filter_codec_opts(o->g->codec_opts, ist->st->codecpar->codec_id, ic,
                            st, ist->dec, &ist->decoder_opts);
    if (ret < 0)
        return ret;
    ...
    if (discard_str) {
        const AVOption *discard_opt =
            av_opt_find(ist->dec_ctx, "skip_frame", NULL, 0, 0);
    ...
    case AVMEDIA_TYPE_SUBTITLE: {
        ...
        if (canvas_size) {
            ret = av_parse_video_size(&ist->dec_ctx->width,
                                      &ist->dec_ctx->height, canvas_size);
            if (ret < 0) {
...
static int is_windows_reserved_device_name(const char *f) {
    ...
    for (const char *p = f; p && *p;) {
        ...
        if (!av_strcasecmp(stem, "AUX") || !av_strcasecmp(stem, "CON") ||
            !av_strcasecmp(stem, "NUL") || !av_strcasecmp(stem, "PRN") ||
            !av_strcasecmp(stem, "COM1") || !av_strcasecmp(stem, "LPT1"))
            return 1;
...
static int safe_filename(const char *f, int allow_subdir) {
    ...
    for (; *f; f++) {
        /* A-Za-z0-9_- */
        if (!((unsigned)((*f | 32) - 'a') < 26 || (unsigned)(*f - '0') < 10 ||
              *f == '_' || *f == '-')) {
            if (f == start)
                return 0;
...
static int dump_attachment(InputStream *ist, const char *filename) {
    ...
    if ((ret = avio_open2(&out, filename, AVIO_FLAG_WRITE, &int_cb, NULL)) <
        0) {
...
int ifile_open(const OptionsContext *o, const char *filename) {
    ...
    if (o->nb_audio_ch_layouts) {
        ...
        if (file_iformat && (priv_class = file_iformat->priv_class) &&
            ...
            av_dict_set(&o->g->format_opts, "ch_layout",
                        o->audio_ch_layouts[o->nb_audio_ch_layouts - 1].u.str,
                        0);
        }
    }
    if (o->nb_frame_rates) {
    ...
    if (o->nb_frame_sizes) {
        av_dict_set(&o->g->format_opts, "video_size",
                    o->frame_sizes[o->nb_frame_sizes - 1].u.str, 0);
    }
    ...
    if (video_codec_name)
        ret =
            err_merge(ret, find_codec(NULL, video_codec_name,
                                      AVMEDIA_TYPE_VIDEO, 0, &ic->video_codec));
    if (audio_codec_name)
        ret =
            err_merge(ret, find_codec(NULL, audio_codec_name,
                                      AVMEDIA_TYPE_AUDIO, 0, &ic->audio_codec));
    if (subtitle_codec_name)
        ret = err_merge(ret, find_codec(NULL, subtitle_codec_name,
                                        AVMEDIA_TYPE_SUBTITLE, 0,
                                        &ic->subtitle_codec));
    if (data_codec_name)
        ret = err_merge(ret, find_codec(NULL, data_codec_name,
                                        AVMEDIA_TYPE_DATA, 0, &ic->data_codec));
    if (ret < 0) {
    ...
    for (i = 0; i < ic->nb_streams; i++) {
        const AVCodec *dummy;
        ret = choose_decoder(o, ic, ic->streams[i], HWACCEL_NONE,
                             AV_HWDEVICE_TYPE_NONE, &dummy);
        if (ret < 0)
            return ret;
    }
    ...
    if (start_time != AV_NOPTS_VALUE) {
        ...
        ret = avformat_seek_file(ic, -1, INT64_MIN, seek_timestamp,
                                 seek_timestamp, 0);
        if (ret < 0) {
    ...
    ret = av_opt_get_int(ic, "use_wallclock_as_timestamps", 0,
    ...
    if (f->readrate) {
        d->readrate_initial_burst =
            o->readrate_initial_burst ? o->readrate_initial_burst : 0.5;
        if (d->readrate_initial_burst < 0.0) {
            av_log(d, AV_LOG_ERROR,
                   "Option -readrate_initial_burst is %0.3f; it must be "
                   "non-negative.\n",
                   d->readrate_initial_burst);
            return AVERROR(EINVAL);
        }
    ...
    while ((e = av_dict_iterate(unused_opts, e))) {
        ...
        if (!(option->flags & AV_OPT_FLAG_DECODING_PARAM)) {
            av_log(d, AV_LOG_ERROR,
                   "Codec AVOption %s (%s) is not a decoding "
                   "option.\n",
    ...
    for (i = 0; i < o->nb_dump_attachment; i++) {
        ...
        for (j = 0; j < f->nb_streams; j++) {
            ...
            if (check_stream_specifier(ic, ist->st,
                                       o->dump_attachment[i].specifier) == 1) {
