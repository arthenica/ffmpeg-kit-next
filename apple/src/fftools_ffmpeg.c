 ...
 * It is derived from FFmpeg's fftools/ffmpeg.c at tag n7.1.5.
 *
...
__thread atomic_uint *nb_output_dumped_ref;
...
__thread Decoder     **decoders;
__thread int        nb_decoders;

static __thread int64_t report_last_time = -1;
static __thread int report_first = 1;
static __thread int64_t keyboard_last_time;
...
static int decode_interrupt_cb(void *ctx)
{
    return received_nb_signals > atomic_load(&transcode_init_done) ||
           cancelRequested(globalSessionId);
}
                           ...
                           double speed)
{
    if (report_callback != NULL) {
        ...
        report_callback(frame_number, fps, quality, total_size,
                        pts < 0 ? -milliseconds : milliseconds,
                        bitrate, speed);
    }
}
...
static void print_report(int is_last_report, int64_t timer_start, int64_t cur_time, int64_t pts)
{
    ...
    if (!print_stats && !is_last_report && !progress_avio && !report_callback)
        return;
    ...
    forward_report(frame_number, fps, quality, total_size, pts, bitrate, speed);
...
static int ffmpeg_var_cleanup(void)
{
    int ret = 0;

    av_freep(&nb_output_dumped_ref);
    nb_output_dumped_ref = av_mallocz(sizeof(*nb_output_dumped_ref));
    if (!nb_output_dumped_ref)
        ret = AVERROR(ENOMEM);
    else
        atomic_init(nb_output_dumped_ref, 0);
...
{
    static char ffmpeg_program_name[] = "ffmpeg";
    Scheduler *sch = NULL;
    ...
    ret = (received_nb_signals || cancelRequested(globalSessionId)) ? 255 :
