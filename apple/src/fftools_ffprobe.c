...
/*
 * This file is the modified version of ffprobe.c file living in ffmpeg source
 ...
 * 01.2020
 * --------------------------------------------------------
 * - ffprobe support (volatile added to critical variables, options updated as
 * ffprobe_options, printf statements replaced with av_log, dropped static
 * variables in methods, show_help_default renamed as show_help_default_ffprobe,
 * implemented exit_program with longjmp, OptionDef defines combined)
...
#define SECTION_FLAG_IS_WRAPPER                                                \
    1 ///< the section only contains other sections, but has no data at its own
      ///< level
#define SECTION_FLAG_IS_ARRAY                                                  \
    2 ///< the section contains an array of elements of the same type
#define SECTION_FLAG_HAS_VARIABLE_FIELDS                                       \
    4 ///< the section may contain a variable number of fields with variable
      ///< keys.
      ///  For these sections the element_name field is mandatory.
#define SECTION_FLAG_HAS_TYPE                                                  \
    8 ///< the section contains a type to distinguish multiple nested elements

    int flags;
    const SectionID
        children_ids[SECTION_MAX_NB_CHILDREN +
                     1]; ///< list of children section IDS, terminated by -1
    const char *element_name; ///< name of the contained element, if provided
    const char
        *unique_name; ///< unique section name, in case the name is ambiguous
    AVDictionary *entries_to_show;
    const char *(*get_type)(
        void *data); ///< function returning a type if defined, must be defined
                     ///< when SECTION_FLAG_HAS_TYPE is defined
...
__thread static struct section sections[] = {
    ...
    [SECTION_ID_ROOT] = {SECTION_ID_ROOT,
                         "root",
                         SECTION_FLAG_IS_WRAPPER,
                         {SECTION_ID_CHAPTERS, SECTION_ID_FORMAT,
                          SECTION_ID_FRAMES, SECTION_ID_PROGRAMS,
                          SECTION_ID_STREAMS, SECTION_ID_PACKETS,
                          SECTION_ID_ERROR, SECTION_ID_PROGRAM_VERSION,
                          SECTION_ID_LIBRARY_VERSIONS, SECTION_ID_PIXEL_FORMATS,
                          -1}},
...
static char *value_string(char *buf, int buf_size, struct unit_value uv) {
    ...
    if (uv.unit == unit_second_str && use_value_sexagesimal_format) {
        ...
        av_strlcatf(buf, buf_size, "%s%s%s",
                    *prefix_string || show_value_unit ? " " : "", prefix_string,
...
struct WriterContext {
    ...
    AVBPrint
        section_pbuf[SECTION_MAX_NB_LEVELS]; ///< generic print buffer
                                             ///< dedicated to each section,
                                             ///  used by various writers

    unsigned int nb_section_packet; ///< number of the packet section in case we
                                    ///< are in "packets_and_frames" section
    unsigned int nb_section_frame;  ///< number of the frame  section in case we
                                    ///< are in "packets_and_frames" section
    unsigned int
        nb_section_packet_frame; ///< nb_section_packet or nb_section_frame
                                 ///< according if is_packets_and_frames
...
static const AVOption writer_options[] = {
    {"string_validation",
     ...
     OFFSET(string_validation),
     AV_OPT_TYPE_INT,
     {.i64 = WRITER_STRING_VALIDATION_REPLACE},
     0,
     WRITER_STRING_VALIDATION_NB - 1,
     .unit = "sv"},
    {"sv",
     "set string validation mode",
     OFFSET(string_validation),
    ...
    {"ignore",
     ...
     0,
     AV_OPT_TYPE_CONST,
     {.i64 = WRITER_STRING_VALIDATION_IGNORE},
     .unit = "sv"},
    {"replace",
     NULL,
     0,
    ...
    {"fail",
     ...
     0,
     AV_OPT_TYPE_CONST,
     {.i64 = WRITER_STRING_VALIDATION_FAIL},
     .unit = "sv"},
...
static int writer_open(WriterContext **wctx, const Writer *writer,
                       const char *args, const struct section *sections,
                       int nb_sections, const char *output) {
    int i, ret = 0;
    ...
    if (args) {
        ...
        if ((ret = av_dict_parse_string(&opts, args, "=", ":", 0)) < 0) {
            av_log(*wctx, AV_LOG_ERROR,
                   "Failed to parse option string '%s' provided to writer "
                   "context\n",
                   args);
            av_dict_free(&opts);
            goto fail;
        ...
        while ((opt = av_dict_iterate(opts, opt))) {
            if ((ret = av_opt_set(*wctx, opt->key, opt->value,
                ...
                av_log(*wctx, AV_LOG_ERROR,
                       "Failed to set option '%s' with value '%s' provided to "
                       "writer "
                       "context\n",
                       opt->key, opt->value);
                av_dict_free(&opts);
...
static inline void writer_print_section_header(WriterContext *wctx, void *data,
                                               int section_id) {
    int parent_section_id;
    wctx->level++;
    av_assert0(wctx->level < SECTION_MAX_NB_LEVELS);
    parent_section_id =
        wctx->level ? (wctx->section[wctx->level - 1])->id : SECTION_ID_NONE;
...
static inline void writer_print_section_footer(WriterContext *wctx) {
    ...
    if (parent_section_id == SECTION_ID_PACKETS_AND_FRAMES) {
        if (section_id == SECTION_ID_PACKET)
            wctx->nb_section_packet++;
        else
            wctx->nb_section_frame++;
    }
...
static inline void writer_print_integer(WriterContext *wctx, const char *key,
                                        long long int val) {
...
static inline int validate_string(WriterContext *wctx, char **dstp,
                                  const char *src) {
    ...
    for (p = (uint8_t *)src; *p;) {
        ...
        if (!invalid ||
            wctx->string_validation == WRITER_STRING_VALIDATION_IGNORE)
            av_bprint_append_data(&dstbuf, p0, p - p0);
    }

    if (invalid_chars_nb &&
        wctx->string_validation == WRITER_STRING_VALIDATION_REPLACE) {
        av_log(
            wctx, AV_LOG_WARNING,
            "%d invalid UTF-8 sequence(s) found in string '%s', replaced with "
            "'%s'\n",
...
static inline int writer_print_string(WriterContext *wctx, const char *key,
...
static inline void writer_print_rational(WriterContext *wctx, const char *key,
...
static void writer_print_time(WriterContext *wctx, const char *key, int64_t ts,
...
#define DEFINE_WRITER_CLASS(name)                                              \
    static const char *name##_get_name(void *ctx) { return #name; }            \
    static const AVClass name##_class = {.class_name = #name,                  \
...
static void default_print_section_header(WriterContext *wctx, void *data) {
    ...
    const struct section *parent_section =
        wctx->level ? wctx->section[wctx->level - 1] : NULL;
    ...
    if (parent_section && !(parent_section->flags & (SECTION_FLAG_IS_WRAPPER |
        ...
        av_bprintf(
            &wctx->section_pbuf[wctx->level],
            "%s%s:", wctx->section_pbuf[wctx->level - 1].str,
...
static void default_print_str(WriterContext *wctx, const char *key,
                              const char *value) {
    DefaultContext *def = wctx->priv;
...
static void default_print_int(WriterContext *wctx, const char *key,
                              long long int value) {
    DefaultContext *def = wctx->priv;
...
static const char *none_escape_str(AVBPrint *dst, const char *src,
                                   const char sep, void *log_ctx) {
    return src;
}
...
static av_cold int compact_init(WriterContext *wctx) {
    ...
    if (strlen(compact->item_sep_str) != 1) {
        av_log(wctx, AV_LOG_ERROR,
               "Item separator '%s' specified, but must contain a single "
               "character\n",
...
static void compact_print_str(WriterContext *wctx, const char *key,
                              const char *value) {
    CompactContext *compact = wctx->priv;
    AVBPrint buf;
    ...
    writer_put_str(wctx,
                   compact->escape_str(&buf, value, compact->item_sep, wctx));
    av_bprint_finalize(&buf, NULL);
}

static void compact_print_int(WriterContext *wctx, const char *key,
                              long long int value) {
    CompactContext *compact = wctx->priv;
...
static const char *flat_escape_key_str(AVBPrint *dst, const char *src,
                                       const char sep) {
    const char *p;
...
static void flat_print_int(WriterContext *wctx, const char *key,
                           long long int value) {
    writer_printf(wctx, "%s%s=%lld\n", wctx->section_pbuf[wctx->level].str, key,
                  value);
}

static void flat_print_str(WriterContext *wctx, const char *key,
                           const char *value) {
    FlatContext *flat = wctx->priv;
    AVBPrint buf;
...
static void ini_print_str(WriterContext *wctx, const char *key,
                          const char *value) {
    AVBPrint buf;
...
static void ini_print_int(WriterContext *wctx, const char *key,
                          long long int value) {
...
static const AVOption json_options[] = {{"compact",
...
static void json_print_section_header(WriterContext *wctx, void *data) {
    ...
    if (section->flags & SECTION_FLAG_IS_WRAPPER) {
        ...
        if (section->flags & SECTION_FLAG_IS_ARRAY) {
            ...
            /* this is required so the parser can distinguish between packets
             * and frames */
            if (parent_section &&
...
static inline void json_print_item_str(WriterContext *wctx, const char *key,
                                       const char *value) {
    AVBPrint buf;
...
static void json_print_str(WriterContext *wctx, const char *key,
                           const char *value) {
    JSONContext *json = wctx->priv;
    const struct section *parent_section =
        wctx->level ? wctx->section[wctx->level - 1] : NULL;
...
static void json_print_int(WriterContext *wctx, const char *key,
                           long long int value) {
    JSONContext *json = wctx->priv;
    const struct section *parent_section =
        wctx->level ? wctx->section[wctx->level - 1] : NULL;
    ...
    av_bprint_init(&buf, 1, AV_BPRINT_SIZE_UNLIMITED);
    writer_printf(wctx, "\"%s\": %lld", json_escape_str(&buf, key, wctx),
                  value);
    av_bprint_finalize(&buf, NULL);
}
...
#define CHECK_COMPLIANCE(opt, opt_name)                                        \
    if (opt) {                                                                 \
        av_log(                                                                \
            wctx, AV_LOG_ERROR,                                                \
            "XSD-compliant output selected but option '%s' was selected, XML " \
            "output may be non-compliant.\n"                                   \
...
static void xml_print_value(WriterContext *wctx, const char *key,
                            const void *value, const int is_int) {
    AVBPrint buf;
    XMLContext *xml = wctx->priv;
    const struct section *section = wctx->section[wctx->level];
    ...
    if (section->flags & SECTION_FLAG_HAS_VARIABLE_FIELDS) {
        xml->indent_level++;
        XML_INDENT();
        av_bprint_escape(&buf, key, NULL, AV_ESCAPE_MODE_XML,
                         AV_ESCAPE_FLAG_XML_DOUBLE_QUOTES);
        writer_printf(wctx, "<%s key=\"%s\"", section->element_name, buf.str);
        av_bprint_clear(&buf);
...
static inline void xml_print_str(WriterContext *wctx, const char *key,
                                 const char *value) {
    xml_print_value(wctx, key, (const void *)value, 0);
}

static inline void xml_print_int(WriterContext *wctx, const char *key,
                                 long long int value) {
...
#define print_val(k, v, u)                                                     \
    do {                                                                       \
        ...
        uv.val.i = v;                                                          \
        uv.unit = u;                                                           \
        writer_print_string(w, k, value_string(val_str, sizeof(val_str), uv),  \
                            0);                                                \
    } while (0)
        ...
        for (int c = 0; c < 3; c++) {
            ...
            for (int i = 0; i < curve->num_pivots - 1; i++) {
                ...
                case AV_DOVI_MAPPING_MMR:
                    ...
                    print_list_fmt("mmr_coef", "%" PRIi64, curve->mmr_order[i],
                                   7, curve->mmr_coef[i][idx][idx2]);
                    break;
                default:
            ...
            if (mapping->nlq_method_idc != AV_DOVI_NLQ_NONE) {
                ...
                switch (mapping->nlq_method_idc) {
                case AV_DOVI_NLQ_LINEAR_DZ:
                    print_int("linear_deadzone_slope",
                              nlq->linear_deadzone_slope);
                    print_int("linear_deadzone_threshold",
...
static void print_dynamic_hdr10_plus(WriterContext *w,
    ...
    for (int n = 1; n < metadata->num_windows; n++) {
        ...
        print_q("window_upper_left_corner_y",
                params->window_upper_left_corner_y, '/');
        print_int("center_of_ellipse_x", params->center_of_ellipse_x);
        print_int("center_of_ellipse_y", params->center_of_ellipse_y);
        print_int("rotation_angle", params->rotation_angle);
        ...
        print_int("overlap_process_option", params->overlap_process_option);
    }
    ...
    for (int n = 0; n < metadata->num_windows; n++) {
        ...
        print_q("fraction_bright_pixels", params->fraction_bright_pixels, '/');
    }
    ...
    for (int n = 0; n < metadata->num_windows; n++) {
        ...
        if (params->tone_mapping_flag) {
            ...
            for (int i = 0; i < params->num_bezier_curve_anchors; i++) {
                print_q("bezier_curve_anchors", params->bezier_curve_anchors[i],
                        '/');
            }
        }
...
static void print_pkt_side_data(WriterContext *w, AVCodecParameters *par,
    ...
    if (sd->type == AV_PKT_DATA_DISPLAYMATRIX && sd->size >= 9 * 4) {
        ...
        if (isnan(rotation))
            rotation = 0;
        writer_print_integers(w, "displaymatrix", sd->data, 9, " %11d", 3, 4,
                              1);
        ...
        if (spherical->projection == AV_SPHERICAL_CUBEMAP) {
            ...
            size_t l, t, r, b;
            av_spherical_tile_bounds(spherical, par->width, par->height, &l, &t,
                                     &r, &b);
...
static int show_log(WriterContext *w, int section_ids, int section_id,
                    int log_level) {
    int i;
    pthread_mutex_lock(&log_mutex);
    if (!log_buffer_size) {
...
static void print_frame_side_data(WriterContext *w, const AVFrame *frame,
    ...
    for (int i = 0; i < frame->nb_side_data; i++) {
        ...
        if (sd->type == AV_FRAME_DATA_DISPLAYMATRIX && sd->size >= 9 * 4) {
            ...
            writer_print_section_header(
                w, NULL, SECTION_ID_FRAME_SIDE_DATA_TIMECODE_LIST);
            for (int j = 1; j <= m; j++) {
                char tcbuf[AV_TIMECODE_STR_SIZE];
                av_timecode_make_smpte_tc_string2(tcbuf, stream->avg_frame_rate,
                                                  tc[j], 0, 0);
                writer_print_section_header(
                    w, NULL, SECTION_ID_FRAME_SIDE_DATA_TIMECODE);
...
static void show_frame(WriterContext *w, AVFrame *frame, AVStream *stream,
    ...
    if (s)
        ...
        print_str_opt("media_type", "unknown");
    ...
    AV_NOWARN_DEPRECATED(print_duration_ts("pkt_duration", frame->pkt_duration);
                         print_duration_time("pkt_duration_time",
                                             frame->pkt_duration,
                                             &stream->time_base);)
#endif
    ...
    if (fd && fd->pkt_pos != -1)
        print_fmt("pkt_pos", "%" PRId64, fd->pkt_pos);
    else
        print_str_opt("pkt_pos", "N/A");
    if (fd && fd->pkt_size != -1)
        print_val("pkt_size", fd->pkt_size, unit_byte_str);
    else
    ...
    case AVMEDIA_TYPE_AUDIO:
        ...
        if (frame->ch_layout.order != AV_CHANNEL_ORDER_UNSPEC) {
            av_channel_layout_describe(&frame->ch_layout, val_str,
                                       sizeof(val_str));
...
static av_always_inline int process_frame(WriterContext *w, InputFile *ifile,
    ...
    if (got_frame) {
        ...
        if (do_show_frames) {
            if (is_sub) {
                ...
                show_frame(w, frame, ifile->streams[pkt->stream_index].st,
                           fmt_ctx);
            }
        }
        if (is_sub) {
...
static void log_read_interval(const ReadInterval *interval, void *log_ctx,
    ...
    if (interval->has_end) {
        av_log(log_ctx, log_level, " end:%s",
               interval->end_is_offset ? "+" : "");
        if (interval->duration_frames)
            av_log(log_ctx, log_level, "#%" PRId64, interval->end);
        else
            av_log(log_ctx, log_level, "%s",
                   av_ts2timestr(interval->end, &AV_TIME_BASE_Q));
    } else {
...
static int read_interval_packets(WriterContext *w, InputFile *ifile,
    ...
    if (interval->has_start) {
        ...
        if ((ret = avformat_seek_file(fmt_ctx, -1, -INT64_MAX, target,
                                      INT64_MAX, 0)) < 0) {
            av_log(NULL, AV_LOG_ERROR,
                   "Could not seek to position %" PRId64 ": %s\n",
...
static int show_stream(WriterContext *w, AVFormatContext *fmt_ctx,
                       int stream_idx, InputStream *ist, int in_program) {
    ...
    writer_print_section_header(
        w, NULL, in_program ? SECTION_ID_PROGRAM_STREAM : SECTION_ID_STREAM);
    ...
    case AVMEDIA_TYPE_VIDEO:
        ...
        if (dec_ctx) {
            ...
            print_int("film_grain",
                      !!(dec_ctx->properties & FF_CODEC_PROPERTY_FILM_GRAIN));
        }
    ...
    case AVMEDIA_TYPE_AUDIO:
        ...
        if (par->ch_layout.order != AV_CHANNEL_ORDER_UNSPEC) {
            av_channel_layout_describe(&par->ch_layout, val_str,
                                       sizeof(val_str));
            print_str("channel_layout", val_str);
        } else {
            print_str_opt("channel_layout", "unknown");
    ...
    if (nb_streams_packets[stream_idx])
        print_fmt("nb_read_packets", "%" PRIu64,
                  nb_streams_packets[stream_idx]);
    else
        print_str_opt("nb_read_packets", "N/A");
    if (do_show_data)
        writer_print_data(w, "extradata", par->extradata, par->extradata_size);
...
#define PRINT_DISPOSITION(flagname, name)                                      \
    ...
    if (do_show_stream_tags)
        ret = show_tags(w, stream->metadata,
                        in_program ? SECTION_ID_PROGRAM_STREAM_TAGS
                                   : SECTION_ID_STREAM_TAGS);
...
static int show_program(WriterContext *w, InputFile *ifile,
                        AVProgram *program) {
    AVFormatContext *fmt_ctx = ifile->fmt_ctx;
    int i, ret = 0;
...
static int open_input_file(InputFile *ifile, const char *filename,
    ...
    for (i = 0; i < fmt_ctx->nb_streams; i++) {
        ...
        }
        {
            ...
            if (do_show_log) {
                // For loging it is needed to disable at least frame threads as
                // otherwise the log information would need to be reordered and
                // matches up to contexts and frames That is in fact possible
                // but not trivial
...
static int parse_read_interval(const char *interval_spec,
    ...
    if (p && *p) {
        ...
        if (interval->end_is_offset && *p == '#') {
            ...
            if (*tail || lli < 0) {
                av_log(NULL, AV_LOG_ERROR,
                       "Invalid or negative value '%s' for duration number of "
                       "frames\n",
                       p);
                goto end;
            }
...
int ffprobe_execute(int argc, char **argv) {
    ...
    OptionDef options[] = {
        ...
        {"show_packets",
         0,
         {.func_arg = &opt_show_packets},
         "show packets info"},
        {"show_programs",
         0,
         {.func_arg = &opt_show_programs},
         "show programs info"},
        {"show_streams",
         0,
         {.func_arg = &opt_show_streams},
         "show streams info"},
        {"show_chapters",
         0,
         {.func_arg = &opt_show_chapters},
         "show chapters info"},
        {"count_frames",
         OPT_BOOL,
         {&do_count_frames},
        ...
        {"show_program_version",
         0,
         {.func_arg = &opt_show_program_version},
         "show ffprobe version"},
        {"show_library_versions",
         0,
         {.func_arg = &opt_show_library_versions},
         "show library versions"},
        {"show_versions",
         0,
         {.func_arg = &opt_show_versions},
         "show program and library versions"},
        {"show_pixel_formats",
         0,
         {.func_arg = &opt_show_pixel_formats},
         "show pixel format descriptions"},
        {"show_optional_fields",
         HAS_ARG,
         {.func_arg = &opt_show_optional_fields},
        ...
        {"read_intervals",
         HAS_ARG,
         {.func_arg = opt_read_intervals},
         "set read intervals",
         "read_intervals"},
        {"i",
         HAS_ARG,
         {.func_arg = opt_input_file_i},
         "read specified file",
         "input_file"},
        {"o",
         HAS_ARG,
         {.func_arg = opt_output_file_o},
         "write to specified output",
         "output_file"},
        {"print_filename",
         HAS_ARG,
         {.func_arg = opt_print_filename},
         "override the printed input filename",
         "print_file"},
        {"find_stream_info",
    ...
    if (savedCode == 0) {
        ...
        if ((ret = writer_open(&wctx, w, w_args, sections,
                               FF_ARRAY_ELEMS(sections), output_filename)) >=
            0) {
            if (w == &xml_writer)
                wctx->string_validation_utf8_flags |=
        ...
        main_ffprobe_return_code = savedCode;
    }
