...
/*
 * This file is the modified version of ffmpeg_dec.c file living in ffmpeg
...
static AVRational audio_samplerate_update(void *logctx, Decoder *d,
    ...
    if (d->last_frame_pts != AV_NOPTS_VALUE)
        d->last_frame_pts =
            av_rescale_q(d->last_frame_pts, d->last_frame_tb, tb_new);
    d->last_frame_duration_est =
        av_rescale_q(d->last_frame_duration_est, d->last_frame_tb, tb_new);
...
static void audio_ts_process(void *logctx, Decoder *d, AVFrame *frame) {
    ...
    pts_pred = d->last_frame_pts == AV_NOPTS_VALUE
                   ? 0
    ...
    frame->pts =
        av_rescale_delta(frame->time_base, frame->pts, tb, frame->nb_samples,
                         &d->last_filter_in_rescale_delta, tb);

    d->last_frame_pts = frame->pts;
    d->last_frame_duration_est = av_rescale_q(frame->nb_samples, tb_filter, tb);
...
static int64_t video_duration_estimate(const InputStream *ist,
    ...
    if (ist->dec_ctx->framerate.den && ist->dec_ctx->framerate.num) {
        ...
        AVRational field_rate =
            av_mul_q(ist->dec_ctx->framerate, (AVRational){2, 1});
        codec_duration =
            av_rescale_q(fields, av_inv_q(field_rate), frame->time_base);
    }
...
static int video_frame_process(InputStream *ist, AVFrame *frame) {
    ...
    if (ist->par->video_delay < ist->dec_ctx->has_b_frames) {
        if (ist->dec_ctx->codec_id == AV_CODEC_ID_H264) {
            ...
            av_log(ist->dec_ctx, AV_LOG_WARNING,
                   ...
                   ist->dec_ctx->has_b_frames, ist->par->video_delay);
    }
    ...
    if (ist->dec_ctx->width != frame->width ||
        ...
        av_log(NULL, AV_LOG_DEBUG,
               ...
               frame->width, frame->height, frame->format, ist->dec_ctx->width,
               ist->dec_ctx->height, ist->dec_ctx->pix_fmt);
    }
    ...
    if (frame->pts == AV_NOPTS_VALUE)
        frame->pts = d->last_frame_pts == AV_NOPTS_VALUE
                         ? 0
                         : d->last_frame_pts + d->last_frame_duration_est;
...
static int process_subtitle(InputStream *ist, AVFrame *frame) {
    ...
    if (ist->fix_sub_duration) {
        AVSubtitle *sub_prev = d->sub_prev[0]->buf[0]
                                   ? (AVSubtitle *)d->sub_prev[0]->buf[0]->data
                                   : NULL;
        int end = 1;
        if (sub_prev) {
            end = av_rescale(subtitle->pts - sub_prev->pts, 1000, AV_TIME_BASE);
    ...
    for (int i = 0; i < ist->nb_filters; i++) {
        ...
        if (ret < 0) {
            av_log(ist, AV_LOG_ERROR,
                   "Error sending a subtitle for filtering: %s\n",
                   av_err2str(ret));
            return ret;
        }
    }
...
int fix_sub_duration_heartbeat(InputStream *ist, int64_t signal_pts) {
    ...
    if (!ist->fix_sub_duration || !prev_subtitle || !prev_subtitle->num_rects ||
...
static int packet_decode(InputStream *ist, AVPacket *pkt, AVFrame *frame) {
    ...
    while (1) {
        ...
        update_benchmark("decode_%s %d.%d", type_desc, ist->file_index,
                         ist->index);
...
static void *decoder_thread(void *arg) {
    ...
    InputStream *ist = (InputStream *)context->arg;
    av_free(arg);
    ...
    while (!input_status) {
        ...
        if (ret == AVERROR_EOF) {
            ...
            if (ist->dec->type == AVMEDIA_TYPE_AUDIO) {
                ...
                dur.duration = av_rescale_q(
                    ist->nb_samples, (AVRational){1, ist->dec_ctx->sample_rate},
                    ist->st->time_base);

                av_thread_message_queue_send(ifile->audio_duration_queue, &dur,
                                             0);
            }
...
static int dec_thread_start(InputStream *ist) {
    ...
    ret = pthread_create(&d->thread, NULL, decoder_thread, context);
    if (ret) {
        ret = AVERROR(ret);
...
static int hw_device_setup_for_decode(InputStream *ist) {
    ...
    if (ist->hwaccel_device) {
        ...
        if (ist->hwaccel_id == HWACCEL_AUTO) {
            ...
            // if "-init_hw_device qsv=name:device" is used. There are 2 QSV
            // devices if both "-qsv_device device" and "-init_hw_device
            // qsv=name:device" are used,
            // hw_device_get_by_type(AV_HWDEVICE_TYPE_QSV) returns NULL. To keep
            // back-compatibility with the removed ad-hoc libmfx setup code,
            // call hw_device_get_by_name("__qsv_device") to select the internal
            // QSV device.
