...
/*
 * This file is the modified version of ffmpeg_mux_init.c file living in ffmpeg
...
static const char *const opt_name_bitstream_filters[] = {"bsf", "absf", "vbsf",
                                                         NULL};
...
static const char *const opt_name_enc_stats_pre_fmt[] = {"enc_stats_pre_fmt",
                                                         NULL};
static const char *const opt_name_enc_stats_post_fmt[] = {"enc_stats_post_fmt",
                                                          NULL};
...
static const char *const opt_name_fix_sub_duration_heartbeat[] = {
    "fix_sub_duration_heartbeat", NULL};
static const char *const opt_name_fps_mode[] = {"fps_mode", NULL};
static const char *const opt_name_force_fps[] = {"force_fps", NULL};
static const char *const opt_name_forced_key_frames[] = {"forced_key_frames",
                                                         NULL};
...
static const char *const opt_name_max_frames[] = {"frames", "aframes",
...
static const char *const opt_name_presets[] = {"pre", "apre", "vpre", "spre",
                                               NULL};
...
static const char *const opt_name_audio_ch_layouts[] = {"channel_layout",
                                                        "ch_layout", NULL};
...
static int choose_encoder(const OptionsContext *o, AVFormatContext *s,
    ...
    *enc = NULL;
    ...
    if (!codec_name) {
        ost->par_in->codec_id =
            av_guess_codec(s->oformat, NULL, s->url, NULL, ost->type);
...
static int unescape(char **pdst, size_t *dst_len, const char **pstr,
                    char delim) {
    const char *str = *pstr;
    char *dst;
    size_t len, idx;
...
static int enc_stats_init(OutputStream *ost, EncStats *es, int pre,
    ...
    while (*next) {
        ...
        if (!val) {
            av_log(NULL, AV_LOG_ERROR, "Empty formatting directive in: %s\n",
                   fmt_spec);
            return AVERROR(EINVAL);
        }

        if (!*next) {
            av_log(NULL, AV_LOG_ERROR, "Missing closing brace in: %s\n",
                   fmt_spec);
            ret = AVERROR(EINVAL);
            goto fail;
        }
        ...
        for (size_t i = 0; i < FF_ARRAY_ELEMS(fmt_specs); i++) {
            if (!strcmp(val, fmt_specs[i].str)) {
                if ((pre && fmt_specs[i].post_only) ||
                    (!pre && fmt_specs[i].pre_only)) {
                    av_log(
                        NULL, AV_LOG_ERROR,
...
static MuxStream *mux_stream_alloc(Muxer *mux, enum AVMediaType type) {
    ...
    ms =
        allocate_array_elem(&mux->of.streams, sizeof(*ms), &mux->of.nb_streams);
    if (!ms)
        return NULL;
...
static enum AVPixelFormat pix_fmt_parse(OutputStream *ost, const char *name) {
    ...
    if (fmts && !fmt_in_list(fmts, fmt)) {
        ...
        if (strcmp(name, name_canonical) &&
            ...
            snprintf(name_other, sizeof(name_other), "%s%ce", name,
                     name_canonical[len - 2] == 'l' ? 'b' : 'l');
...
static int new_stream_video(Muxer *mux, const OptionsContext *o,
    ...
    if (ost->enc_ctx) {
        ...
        if (frame_size) {
            ret = av_parse_video_size(&video_enc->width, &video_enc->height,
                                      frame_size);
            if (ret < 0) {
        ...
        MATCH_PER_STREAM_OPT(chroma_intra_matrices, str, chroma_intra_matrix,
                             oc, st);
        ...
        for (i = 0; p; i++) {
            ...
            video_enc->rc_override = av_realloc_array(
                video_enc->rc_override, i + 1, sizeof(RcOverride));
        ...
        ost->is_cfr = (ost->vsync_method == VSYNC_CFR ||
                       ost->vsync_method == VSYNC_VSCFR);
    }
...
static int new_stream_audio(Muxer *mux, const OptionsContext *o,
    ...
    if (ost->enc_ctx) {
        ...
        if (layout) {
            if (av_channel_layout_from_string(&audio_enc->ch_layout, layout) <
                0) {
        ...
        if (sample_fmt && (audio_enc->sample_fmt = av_get_sample_fmt(
        ...
        for (int n = 0; n < o->nb_audio_channel_maps; n++) {
            ...
            if ((map->ofile_idx == -1 || ost->file_index == map->ofile_idx) &&
                ...
                if (map->channel_idx == -1) {
                    ...
                    av_log(ost, AV_LOG_FATAL,
                           "Cannot determine input stream for channel mapping "
                           "%d.%d\n",
...
static int new_stream_subtitle(Muxer *mux, const OptionsContext *o,
    ...
    if (ost->enc_ctx) {
        ...
        if (frame_size) {
            int ret = av_parse_video_size(&subtitle_enc->width,
                                          &subtitle_enc->height, frame_size);
            if (ret < 0) {
                av_log(ost, AV_LOG_FATAL, "Invalid frame size: %s.\n",
                       frame_size);
                return ret;
...
static int streamcopy_init(const Muxer *mux, OutputStream *ost) {
    ...
    if (ret < 0) {
        av_log(ost, AV_LOG_FATAL, "Error setting up codec context options.\n");
        goto fail;
    }
    ...
    ret = avformat_transfer_internal_stream_timing_info(
        mux->fc->oformat, ost->st, ist->st, copy_tb);
    if (ret < 0)
        goto fail;
    ...
    if (!ms->copy_prior_start) {
        ms->ts_copy_start =
            (mux->of.start_time == AV_NOPTS_VALUE) ? 0 : mux->of.start_time;
        if (copy_ts && ifile->start_time != AV_NOPTS_VALUE) {
            ms->ts_copy_start =
                FFMAX(ms->ts_copy_start, ifile->start_time + ifile->ts_offset);
        }
    }
    ...
    if (ost->rotate_overridden) {
        AVPacketSideData *sd = av_packet_side_data_new(
            ...
            &ost->st->codecpar->nb_coded_side_data, AV_PKT_DATA_DISPLAYMATRIX,
            sizeof(int32_t) * 9, 0);
        if (sd)
            av_display_rotation_set((int32_t *)sd->data,
    ...
    case AVMEDIA_TYPE_AUDIO:
        if ((par->block_align == 1 || par->block_align == 1152 ||
             par->block_align == 576) &&
...
static int ost_add(Muxer *mux, const OptionsContext *o, enum AVMediaType type,
    ...
    if (ist)
        av_log(ost, AV_LOG_VERBOSE, "input stream %d:%d", ist->file_index,
               ist->index);
    else if (ofilter)
    ...
    if (ost->enc_ctx) {
        ...
        ret = filter_codec_opts(o->g->codec_opts, enc->codec_id, oc, st,
                                enc->codec, &ost->encoder_opts);
        if (ret < 0)
            return ret;
        ...
        if (preset &&
            (!(ret = get_preset_file_2(preset, enc->codec->name, &s)))) {
            AVBPrint bprint;
    ...
    if (o->bitexact) {
        ...
        ost->bitexact = check_opt_bitexact(ost->enc_ctx, ost->encoder_opts,
    ...
    if (time_base) {
        AVRational q;
        if (av_parse_ratio(&q, time_base, INT_MAX, 0, NULL) < 0 || q.num <= 0 ||
            q.den <= 0) {
            av_log(ost, AV_LOG_FATAL, "Invalid time base: %s\n", time_base);
            return AVERROR(EINVAL);
        }
    ...
    for (i = 0; i < o->nb_max_frames; i++) {
        ...
        if (!*p && type != AVMEDIA_TYPE_VIDEO) {
            av_log(
                ost, AV_LOG_WARNING,
                "Applying unspecific -frames to non video streams, maybe you "
                "meant -vframes ?\n");
            break;
        }
    }
    ...
    MATCH_PER_STREAM_OPT(fix_sub_duration_heartbeat, i,
                         ost->fix_sub_duration_heartbeat, oc, st);
    ...
    if (ost->enc &&
        ...
        if (ret < 0) {
            av_log(ost, AV_LOG_ERROR, "Error binding an input stream\n");
            return ret;
        }
    }
...
static int map_auto_video(Muxer *mux, const OptionsContext *o) {
    ...
    if (av_guess_codec(oc->oformat, NULL, oc->url, NULL, AVMEDIA_TYPE_VIDEO) ==
        AV_CODEC_ID_NONE)
        return 0;
    ...
    for (int j = 0; j < nb_input_files; j++) {
        ...
        if (file_best_ist) {
            if ((qcr == MKTAG('A', 'P', 'I', 'C')) ||
                ...
                file_best_score -= 5000000 * !!(file_best_ist->st->disposition &
...
static int map_auto_audio(Muxer *mux, const OptionsContext *o) {
    ...
    /* audio: most channels */
    if (av_guess_codec(oc->oformat, NULL, oc->url, NULL, AVMEDIA_TYPE_AUDIO) ==
        AV_CODEC_ID_NONE)
        return 0;
...
static int map_auto_data(Muxer *mux, const OptionsContext *o) {
    ...
    enum AVCodecID codec_id =
        av_guess_codec(oc->oformat, NULL, oc->url, NULL, AVMEDIA_TYPE_DATA);
...
static int map_manual(Muxer *mux, const OptionsContext *o,
                      const StreamMap *map) {
    InputStream *ist;
    int ret;
...
static int of_add_attachments(Muxer *mux, const OptionsContext *o) {
    ...
    for (int i = 0; i < o->nb_attachments; i++) {
        ...
        if (err < 0)
            ...
            av_log(mux, AV_LOG_FATAL,
                   "Could not read all %" PRId64 " bytes for "
                   "attachment file %s\n",
                   len, o->attachments[i]);
            err = AVERROR(EIO);
        }
...
static int create_streams(Muxer *mux, const OptionsContext *o) {
    ...
    int auto_disable = o->video_disable * (1 << AVMEDIA_TYPE_VIDEO) |
    ...
    for (int i = 0; i < nb_filtergraphs; i++) {
        ...
        for (int j = 0; j < fg->nb_outputs; j++) {
            ...
            av_log(mux, AV_LOG_VERBOSE,
                   ...
                   "output of complex filtergraph %d.",
                   fg->index);
            if (!o->nb_stream_maps)
                av_log(mux, AV_LOG_VERBOSE,
...
#define IS_INTERLEAVED(type) (type != AVMEDIA_TYPE_ATTACHMENT)
    ...
    for (int i = 0; i < oc->nb_streams; i++) {
        ...
        nb_audio_fs += (ost->enc_ctx && type == AVMEDIA_TYPE_AUDIO &&
                        !(ost->enc_ctx->codec->capabilities &
    ...
    if (!((nb_interleaved > 1 && of->shortest) ||
          (nb_interleaved > 0 && limit_frames) || nb_audio_fs))
        return 0;
    ...
    if ((of->shortest && nb_av_enc > 1) || limit_frames_av_enc || nb_audio_fs) {
        ...
        for (int i = 0; i < oc->nb_streams; i++) {
            ...
            ost->sq_idx_encode = sq_add_stream(
                of->sq_encode, of->shortest || ms->max_frames < INT64_MAX);
    ...
    if (nb_interleaved > nb_av_enc) {
        ...
        for (int i = 0; i < oc->nb_streams; i++) {
            ...
            ost->sq_idx_mux = sq_add_stream(
                mux->sq_mux, of->shortest || ms->max_frames < INT64_MAX);
            if (ost->sq_idx_mux < 0)
 ...
 * @param type metadata type is written here --
 * g(lobal)/s(tream)/c(hapter)/p(rogram)
 * @param index for type c/p, chapter/program index is written here
 * @param stream_spec for type s, the stream specifier is written here
 */
static int parse_meta_type(void *logctx, const char *arg, char *type,
                           int *index, const char **stream_spec) {
    if (*arg) {
        *type = *arg;
        switch (*arg) {
        case 'g':
...
static int of_add_metadata(OutputFile *of, AVFormatContext *oc,
    ...
    for (int i = 0; i < o->nb_metadata; i++) {
        ...
        *val++ = 0;
        ...
        ret = parse_meta_type(of, o->metadata[i].specifier, &type, &index,
                              &stream_spec);
        if (ret < 0)
            return ret;
...
static int copy_chapters(InputFile *ifile, OutputFile *ofile,
                         AVFormatContext *os, int copy_metadata) {
    AVFormatContext *is = ifile->ctx;
    AVChapter **tmp;
    int i;

    tmp = av_realloc_f(os->chapters, is->nb_chapters + os->nb_chapters,
                       sizeof(*os->chapters));
    if (!tmp)
        return AVERROR(ENOMEM);
    os->chapters = tmp;
    ...
    for (i = 0; i < is->nb_chapters; i++) {
        ...
        int64_t start_time =
            (ofile->start_time == AV_NOPTS_VALUE) ? 0 : ofile->start_time;
        int64_t ts_off = av_rescale_q(start_time - ifile->ts_offset,
                                      AV_TIME_BASE_Q, in_ch->time_base);
        int64_t rt = (ofile->recording_time == INT64_MAX)
                         ? INT64_MAX
...
static int copy_metadata(Muxer *mux, AVFormatContext *ic, const char *outspec,
                         const char *inspec, int *metadata_global_manual,
...
#define SET_DICT(type, meta, context, index)                                   \
    ...
    if (type_in == 's') {
        for (i = 0; i < ic->nb_streams; i++) {
            if ((ret = check_stream_specifier(ic, ic->streams[i],
                                              istream_spec)) > 0) {
    ...
    if (type_out == 's') {
        for (i = 0; i < oc->nb_streams; i++) {
            if ((ret = check_stream_specifier(oc, oc->streams[i],
                                              ostream_spec)) > 0) {
...
static int copy_meta(Muxer *mux, const OptionsContext *o) {
    ...
    for (int i = 0; i < o->nb_metadata_map; i++) {
        ...
        ret = copy_metadata(
            mux, in_file_index >= 0 ? input_files[in_file_index]->ctx : NULL,
    ...
    if (!metadata_streams_manual)
        for (int i = 0; i < of->nb_streams; i++) {
            ...
            if (!ost->ist) /* this is true e.g. for attached files */
                continue;
            av_dict_copy(&ost->st->metadata, ost->ist->st->metadata,
                         AV_DICT_DONT_OVERWRITE);
            if (ost->enc_ctx) {
...
static int set_dispositions(Muxer *mux, const OptionsContext *o) {
    ...
    if (have_manual) {
        ...
        // For each media type with more than one stream, find a suitable stream
        // to mark as default, unless one is already marked default. "Suitable"
        // means the first of that type, skipping attached pictures.
        for (int i = 0; i < ctx->nb_streams; i++) {
            OutputStream *ost = of->streams[i];
            enum AVMediaType type = ost->type;
...
const char *const forced_keyframes_const_names[] = {
    "n", "n_forced", "prev_forced_n", "prev_forced_t", "t", NULL};
    ...
    for (i = 0; i < n; i++) {
        ...
        if (strstr(p, "chapters") == p) {
            ...
            for (j = 0; j < nb_ch; j++) {
                ...
                pts[index++] =
                    av_rescale_q(c->start, c->time_base, AV_TIME_BASE_Q) + t;
            }
...
static int process_forced_keyframes(Muxer *mux, const OptionsContext *o) {
    for (int i = 0; i < mux->of.nb_streams; i++) {
        ...
        MATCH_PER_STREAM_OPT(forced_key_frames, str, forced_keyframes, mux->fc,
                             ost->st);

        if (!(ost->type == AVMEDIA_TYPE_VIDEO && ost->enc_ctx &&
              forced_keyframes))
            continue;
        ...
        if (!strncmp(forced_keyframes, "expr:", 5)) {
            ...
            int ret =
                parse_forced_key_frames(ost, &ost->kf, mux, forced_keyframes);
            if (ret < 0)
                return ret;
        }
    }
...
static int validate_enc_avopt(Muxer *mux, const AVDictionary *codec_avopt) {
    ...
    while ((e = av_dict_iterate(unused_opts, e))) {
        const AVOption *option =
            av_opt_find(&class, e->key, NULL, 0,
                        AV_OPT_SEARCH_CHILDREN | AV_OPT_SEARCH_FAKE_OBJ);
        const AVOption *foption =
            av_opt_find(&fclass, e->key, NULL, 0,
                        AV_OPT_SEARCH_CHILDREN | AV_OPT_SEARCH_FAKE_OBJ);
        if (!option || foption)
            continue;
...
static Muxer *mux_alloc(void) {
    Muxer *mux =
        allocate_array_elem(&output_files, sizeof(*mux), &nb_output_files);
...
int of_open(const OptionsContext *o, const char *filename) {
    ...
    if (stop_time != INT64_MAX && recording_time == INT64_MAX) {
        int64_t start_time =
            o->start_time == AV_NOPTS_VALUE ? 0 : o->start_time;
    ...
    mux->thread_queue_size =
        o->thread_queue_size > 0 ? o->thread_queue_size : 8;
    ...
    if (o->bitexact) {
        ...
        of->bitexact =
            check_opt_bitexact(oc, mux->opts, "fflags", AVFMT_FLAG_BITEXACT);
    }
    ...
    if (oc->oformat->flags & AVFMT_NEEDNUMBER &&
        !av_filename_number_test(oc->url)) {
        av_log(mux, AV_LOG_FATAL,
    ...
    if (!(oc->oformat->flags & AVFMT_NOFILE)) {
        ...
        if ((err = avio_open2(&oc->pb, filename, AVIO_FLAG_WRITE,
                              &oc->interrupt_callback, &mux->opts)) < 0) {
    ...
    if (o->mux_preload) {
        av_dict_set_int(&mux->opts, "preload", o->mux_preload * AV_TIME_BASE,
                        0);
    }
