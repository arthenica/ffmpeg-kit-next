 ...
 * Derived from FFmpeg source file fftools/ffmpeg_opt.c.
 *
 * FFmpegKitNext modifications:
 * Copyright (c) 2018-2020, 2026 Taner Sener
 * Copyright (c) 2023-2024 ARTHENICA LTD
 *
 * This modified file is part of FFmpegKitNext.
 * It is derived from FFmpeg's fftools/ffmpeg_opt.c at tag n9.0.1.
 *
...
__thread int filter_buffered_frames = 0;
__thread int vstats_version = 2;
__thread int print_graphs = 0;
__thread char *print_graphs_file = NULL;
__thread char *print_graphs_format = NULL;
...
int assert_file_overwrite(const char *filename)
{
    ...
    if (file_overwrite && no_file_overwrite) {
        av_log(NULL, AV_LOG_STDERR, "Error, both -y and -n supplied. Exiting.\n");
        return AVERROR(EINVAL);
    }
    ...
    if (!file_overwrite) {
        if (proto_name && !strcmp(proto_name, "file") && avio_check(filename, 0) == 0) {
            if (stdin_interaction && !no_file_overwrite) {
                av_log(NULL, AV_LOG_STDERR,"File '%s' already exists. Overwrite? [y/N] ", filename);
                fflush(stderr);
                term_exit();
...
static void show_help_default_ffmpeg(const char *opt, const char *arg)
{
...
static int show_help_ffmpeg(void *optctx, const char *opt, const char *arg)
{
    return show_help_common(optctx, opt, arg, show_help_default_ffmpeg);
}
...
__thread const OptionDef *options;
static __thread OptionDef *options_local;

void ffmpeg_options_uninit(void)
{
    av_freep(&options_local);
    options = NULL;
}

void ffmpeg_options_init(void)
{
    OptionDef init_options[] = {
    /* main options */
    CMDUTILS_COMMON_OPTIONS(show_help_ffmpeg)
