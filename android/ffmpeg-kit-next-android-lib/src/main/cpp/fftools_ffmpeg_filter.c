...
/*
 * This file is the modified version of ffmpeg_filter.c file living in ffmpeg
...
typedef struct OutputFilterPriv {
    ...
    // multiple options They point directly to the relevant lists of the
    // encoder.
    const int *formats;
...
static void sub2video_update(InputFilterPriv *ifp, int64_t heartbeat_pts,
    ...
    if (sub) {
        ...
        pts =
            ifp->sub2video.initialize ? heartbeat_pts : ifp->sub2video.end_pts;
        end_pts = INT64_MAX;
        num_rects = 0;
    }
    if (sub2video_get_blank_frame(ifp) < 0) {
        av_log(NULL, AV_LOG_ERROR, "Impossible to get a blank canvas.\n");
        return;
    }
    dst = frame->data[0];
...
static int choose_pix_fmts(OutputFilter *ofilter, AVBPrint *bprint,
    ...
    *dst = NULL;
    ...
    if (ost->keep_pix_fmt || ofp->format != AV_PIX_FMT_NONE) {
        *dst = ofp->format == AV_PIX_FMT_NONE
                   ? NULL
        ...
        for (; *p != AV_PIX_FMT_NONE; p++) {
            ...
            av_bprintf(bprint, "%s%c", name,
                       p[1] == AV_PIX_FMT_NONE ? '\0' : '|');
        }
...
static int filter_opt_apply(AVFilterContext *f, const char *key,
                            const char *val) {
...
static int set_channel_layout(OutputFilterPriv *f, OutputStream *ost) {
    ...
    if (!c->ch_layouts) {
        /* Use the default native layout for the requested amount of channels
           when the encoder doesn't have a list of supported layouts */
    ...
    case AVMEDIA_TYPE_VIDEO:
        ...
        if (ost->enc_ctx->pix_fmt != AV_PIX_FMT_NONE) {
            ...
            if (!strcmp(c->name, "mjpeg")) {
                ...
                // range, yet the latter information isn't available here.
                ...
                if (strict) {
                    const AVOption *o =
                        av_opt_find(ost->enc_ctx, strict->key, NULL, 0, 0);
                    av_assert0(o);
        ...
        ofp->fps.framerate_supported =
            ost->force_fps ? NULL : c->supported_framerates;
...
int init_simple_filtergraph(InputStream *ist, OutputStream *ost,
    ...
    snprintf(fgp->log_name, sizeof(fgp->log_name), "%cf#%d:%d",
             av_get_media_type_string(ost->type)[0], ost->file_index,
             ost->index);
    ...
    if (ifp->linklabel) {
        ...
        for (i = 0; i < s->nb_streams; i++) {
            ...
            if (stream_type != type &&
                ...
                continue;
            if (check_stream_specifier(s, s->streams[i],
                                       *p == ':' ? p + 1 : p) == 1) {
                st = s->streams[i];
                break;
            }
...
static int insert_trim(int64_t start_time, int64_t duration,
    ...
    if (duration != INT64_MAX) {
        ret =
            av_opt_set_int(ctx, "durationi", duration, AV_OPT_SEARCH_CHILDREN);
    }
...
static int configure_output_video_filter(FilterGraph *fg, OutputFilter *ofilter,
    ...
    if ((ofp->width || ofp->height) && ofilter->ost->autoscale) {
        ...
        snprintf(args, sizeof(args), "%d:%d", ofp->width, ofp->height);
        ...
        if ((ret = avfilter_graph_create_filter(
                 &filter, avfilter_get_by_name("scale"), name, args, NULL,
                 fg->graph)) < 0)
            return ret;
    ...
    ret = insert_trim(of->start_time, of->recording_time, &last_filter,
                      &pad_idx, name);
    if (ret < 0)
        return ret;
...
static int configure_output_audio_filter(FilterGraph *fg, OutputFilter *ofilter,
                                         AVFilterInOut *out) {
    OutputFilterPriv *ofp = ofp_from_ofilter(ofilter);
    OutputStream *ost = ofilter->ost;
    OutputFile *of = output_files[ost->file_index];
...
#define AUTO_INSERT_FILTER(opt_name, filter_name, arg)                         \
    do {                                                                       \
        ...
        av_log(fg, AV_LOG_INFO,                                                \
               opt_name " is forwarded to lavfi "                              \
...
static int configure_input_video_filter(FilterGraph *fg, InputFilter *ifilter,
    ...
    av_bprintf(&args,
               ...
               "pixel_aspect=%d/%d",
               ifp->width, ifp->height, ifp->format, ifp->time_base.num,
               ifp->time_base.den, sar.num, sar.den);
    if (fr.num && fr.den)
    ...
    snprintf(name, sizeof(name), "trim_in_%d_%d", ist->file_index, ist->index);
    if (copy_ts) {
...
static int configure_input_audio_filter(FilterGraph *fg, InputFilter *ifilter,
    ...
    av_bprintf(&args, "time_base=%d/%d:sample_rate=%d:sample_fmt=%s",
               ifp->time_base.num, ifp->time_base.den, ifp->sample_rate,
    ...
    if ((ret = avfilter_graph_create_filter(&ifp->filter, abuffer_filt, name,
                                            args.str, NULL, fg->graph)) < 0)
        return ret;
    last_filter = ifp->filter;
...
static int configure_filtergraph(FilterGraph *fg) {
    ...
    if (simple) {
        ...
        if (av_dict_count(ost->sws_dict)) {
            ret = av_dict_get_string(ost->sws_dict, &fg->graph->scale_sws_opts,
                                     '=', ':');
            if (ret < 0)
                goto fail;
        }
    ...
    if ((ret = graph_parse(fg->graph, graph_desc, &inputs, &outputs,
                           hw_device)) < 0)
        goto fail;
    ...
    for (i = 0; i < fg->nb_inputs; i++) {
        ...
        while (av_fifo_read(ifp->frame_queue, &tmp, 1) >= 0) {
            if (ifp->type_src == AVMEDIA_TYPE_SUBTITLE) {
                sub2video_update(ifp, INT64_MIN,
                                 (const AVSubtitle *)tmp->buf[0]->data);
            } else {
...
void fg_send_command(FilterGraph *fg, double time, const char *target,
    ...
    if (time < 0) {
        ...
        ret = avfilter_graph_send_command(
            fg->graph, target, command, arg, response, sizeof(response),
            all_filters ? 0 : AVFILTER_CMD_FLAG_ONE);
        fprintf(stderr, "Command reply for stream %d: ret:%d res:\n%s",
                fg->index, ret, response);
    } else if (!all_filters) {
        fprintf(stderr,
                "Queuing commands only on filters supporting the specific "
...
static int choose_out_timebase(OutputFilterPriv *ofp, AVFrame *frame) {
    ...
    if (ofilter->ost->is_cfr) {
        ...
        if (fps->framerate_max.num &&
            (av_q2d(fr) > av_q2d(fps->framerate_max) || !fr.den))
    ...
    if (fr.num > 0) {
        ...
        if (fps->framerate_clip) {
            av_reduce(&fr.num, &fr.den, fr.num, fr.den, fps->framerate_clip);
        }
    }
...
early_exit:
    ...
    if (debug_ts) {
        av_log(NULL, AV_LOG_INFO,
               ...
               av_ts2timestr(frame->pts, &tb_dst), float_pts, tb_dst.num,
               tb_dst.den);
    }
...
static void video_sync_process(OutputFilterPriv *ofp, AVFrame *frame,
    ...
    if (!frame) {
        *nb_frames_prev = *nb_frames =
            mid_pred(fps->frames_prev_hist[0], fps->frames_prev_hist[1],
    ...
    sync_ipts =
        adjust_frame_pts_to_encoder_tb(frame, ofp->tb_out, ofp->ts_offset);
...
finish:
    memmove(fps->frames_prev_hist + 1, fps->frames_prev_hist,
    ...
    if (*nb_frames > (*nb_frames_prev && fps->last_dropped) +
                         (*nb_frames > *nb_frames_prev)) {
        if (*nb_frames > dts_error_threshold * 30) {
            av_log(ost, AV_LOG_ERROR,
                   "%" PRId64 " frame duplication too large, skipping\n",
                   *nb_frames - 1);
            ofilter->nb_frames_drop++;
            *nb_frames = 0;
            return;
        }
        ofilter->nb_frames_dup += *nb_frames -
                                  (*nb_frames_prev && fps->last_dropped) -
                                  (*nb_frames > *nb_frames_prev);
        av_log(ost, AV_LOG_VERBOSE, "*** %" PRId64 " dup!\n", *nb_frames - 1);
        if (ofilter->nb_frames_dup > fps->dup_warning) {
            av_log(ost, AV_LOG_WARNING,
                   "More than %" PRIu64 " frames duplicated\n",
                   fps->dup_warning);
            fps->dup_warning *= 10;
        }
    }
    ...
    fps->dropped_keyframe |=
        fps->last_dropped && (frame->flags & AV_FRAME_FLAG_KEY);
}
...
static int fg_output_frame(OutputFilterPriv *ofp, AVFrame *frame) {
    ...
    for (int64_t i = 0; i < nb_frames; i++) {
        ...
        if (type == AVMEDIA_TYPE_VIDEO) {
            AVFrame *frame_in =
                (i < nb_frames_prev && frame_prev->buf[0]) ? frame_prev : frame;
            if (!frame_in)
                break;
            ...
            frame->pts =
                (frame->pts == AV_NOPTS_VALUE)
                    ...
                    : av_rescale_q(frame->pts, frame->time_base, ofp->tb_out) -
                          av_rescale_q(ofp->ts_offset, AV_TIME_BASE_Q,
                                       ofp->tb_out);

            frame->time_base = ofp->tb_out;
            frame->duration =
                av_rescale_q(frame->nb_samples,
                             (AVRational){1, frame->sample_rate}, ofp->tb_out);
    ...
    if (ost->type == AVMEDIA_TYPE_VIDEO) {
        if (!frame->duration) {
            ...
            if (fr.num > 0 && fr.den > 0)
                frame->duration =
                    av_rescale_q(1, av_inv_q(fr), frame->time_base);
        }
...
int ifilter_sub2video(InputFilter *ifilter, const AVFrame *frame) {
    ...
    if (ifilter->graph->graph) {
        ...
        sub2video_update(ifp, INT64_MIN,
                         (const AVSubtitle *)frame->buf[0]->data);
    } else if (frame) {
...
int ifilter_send_frame(InputFilter *ifilter, AVFrame *frame,
    ...
    frame->duration =
        av_rescale_q(frame->duration, frame->time_base, ifp->time_base);
    frame->time_base = ifp->time_base;
#if LIBAVUTIL_VERSION_MAJOR < 59
    AV_NOWARN_DEPRECATED(frame->pkt_duration = frame->duration;)
#endif
