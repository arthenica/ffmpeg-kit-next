...
/**
 * Tests for [FFmpegKit] class.
 */
class FFmpegKitTest {

    @Test
    fun mediaInformationMp3() {
        val mediaInformation = MediaInformationJsonParser.from(MEDIA_INFORMATION_MP3)
        ...
        assertAudioStream(mediaInformation.getStreams()[0], 0L, "mp3", "MP3 (MPEG audio layer 3)", "44100", "stereo", "fltp", "320000")

        Assert.assertNotNull(mediaInformation.getChapters())
        Assert.assertEquals(7, mediaInformation.getChapters().size)
        assertChapter(mediaInformation.getChapters()[0], 0L, "1/22050", 0L, "0.000000", 11158238L, "506.042540")
        assertChapter(mediaInformation.getChapters()[1], 1L, "1/22050", 11158238L, "506.042540", 21433051L, "972.020454")
    }

    @Test
    fun mediaInformationJpg() {
        val mediaInformation = MediaInformationJsonParser.from(MEDIA_INFORMATION_JPG)
        ...
        Assert.assertNotNull(mediaInformation.getStreams())
        Assert.assertEquals(1, mediaInformation.getStreams().size)
        assertVideoStream(mediaInformation.getStreams()[0], 0L, "mjpeg", "Motion JPEG", "yuvj444p", 1496L, 1729L, "1:1", "1496:1729", null, "0/0", "25/1", "1/25", "0/1")
    }

    @Test
    fun mediaInformationGif() {
        val mediaInformation = MediaInformationJsonParser.from(MEDIA_INFORMATION_GIF)
        ...
        Assert.assertNotNull(mediaInformation.getStreams())
        Assert.assertEquals(1, mediaInformation.getStreams().size)
        assertVideoStream(mediaInformation.getStreams()[0], 0L, "gif", "CompuServe GIF (Graphics Interchange Format)", "bgra", 400L, 400L, null, null, null, "133/12", "100/9", "1/100", "12/133")
    }

    @Test
    fun mediaInformationMp4() {
        val mediaInformation = MediaInformationJsonParser.from(MEDIA_INFORMATION_MP4)
        ...
        Assert.assertNotNull(mediaInformation.getStreams())
        Assert.assertEquals(1, mediaInformation.getStreams().size)
        assertVideoStream(mediaInformation.getStreams()[0], 0L, "h264", "H.264 / AVC / MPEG-4 AVC / MPEG-4 part 10", "yuv420p", 1280L, 720L, "1:1", "16:9", "9166570", "30/1", "30/1", "1/15360", "1/60")

        assertStreamTag(mediaInformation.getStreams()[0], "language", "und")
        assertStreamTag(mediaInformation.getStreams()[0], "handler_name", "VideoHandler")
    }

    @Test
    fun mediaInformationPng() {
        val mediaInformation = MediaInformationJsonParser.from(MEDIA_INFORMATION_PNG)
        ...
        Assert.assertNotNull(mediaInformation.getStreams())
        Assert.assertEquals(1, mediaInformation.getStreams().size)
        assertVideoStream(mediaInformation.getStreams()[0], 0L, "png", "PNG (Portable Network Graphics) image", "pal8", 1198L, 1198L, "1:1", "1:1", null, "0/0", "25/1", "1/25", "0/1")
    }

    @Test
    fun mediaInformationOgg() {
        val mediaInformation = MediaInformationJsonParser.from(MEDIA_INFORMATION_OGG)
        ...
        assertVideoStream(mediaInformation.getStreams()[0], 0L, "theora", "Theora", "yuv420p", 1920L, 1080L, null, null, null, "25/1", "25/1", "1/25", "1/25")
        assertAudioStream(mediaInformation.getStreams()[1], 1L, "vorbis", "Vorbis", "48000", "stereo", "fltp", "80000")

        assertStreamTag(mediaInformation.getStreams()[0], "ENCODER", "ffmpeg2theora 0.19")
        assertStreamTag(mediaInformation.getStreams()[1], "ENCODER", "ffmpeg2theora 0.19")
    }

    @Test
    fun parseSimpleCommand() {
        val argumentArray = FFmpegKitConfig.parseArguments("-hide_banner -loop 1 -i file.jpg -filter_complex [0:v]setpts=PTS-STARTPTS[video] -map [video] -vsync 2 -async 1 video.mp4")

        Assert.assertNotNull(argumentArray)
        Assert.assertEquals(14, argumentArray.size)
    ...
    @Test
    fun parseSingleQuotesInCommand() {
        val argumentArray = FFmpegKitConfig.parseArguments("-loop 1 'file one.jpg'  -filter_complex  '[0:v]setpts=PTS-STARTPTS[video]'  -map  [video]  video.mp4 ")

        Assert.assertNotNull(argumentArray)
        Assert.assertEquals(8, argumentArray.size)
    ...
    @Test
    fun parseDoubleQuotesInCommand() {
        var argumentArray = FFmpegKitConfig.parseArguments("-loop  1 \"file one.jpg\"   -filter_complex \"[0:v]setpts=PTS-STARTPTS[video]\"  -map  [video]  video.mp4 ")
        ...
        Assert.assertNotNull(argumentArray)
        Assert.assertEquals(13, argumentArray.size)
    ...
    @Test
    fun parseDoubleQuotesAndEscapesInCommand() {
        var argumentArray = FFmpegKitConfig.parseArguments("  -i   file:///tmp/input.mp4 -vf \"subtitles=file:///tmp/subtitles.srt:force_style=\'FontSize=16,PrimaryColour=&HFFFFFF&\'\" -vcodec libx264   -acodec copy  -q:v 0 -q:a  0  video.mp4")
    ...
    @Test
    fun parseEmptyAndWhitespaceOnlyCommands() {
        Assert.assertArrayEquals(emptyArray<String>(), FFmpegKitConfig.parseArguments(""))
        Assert.assertArrayEquals(emptyArray<String>(), FFmpegKitConfig.parseArguments("     "))
    }

    @Test
    fun parseQuotesEmbeddedInsideArguments() {
        val argumentArray = FFmpegKitConfig.parseArguments("-metadata title='A B' -metadata comment=\"C D\" 'one'\"two\"")

        Assert.assertArrayEquals(
            arrayOf("-metadata", "title=A B", "-metadata", "comment=C D", "onetwo"),
            argumentArray
        )
    }

    @Test
    fun parseKeepsQuotesInsideOppositeQuoteType() {
        val argumentArray = FFmpegKitConfig.parseArguments("'single with \"double\" quotes' \"double with 'single' quotes\"")

        Assert.assertArrayEquals(
            arrayOf("single with \"double\" quotes", "double with 'single' quotes"),
            argumentArray
        )
    }

    @Test
    fun parseUnterminatedQuoteKeepsAccumulatedArgument() {
        val argumentArray = FFmpegKitConfig.parseArguments("-i 'file one.mp4")

        Assert.assertArrayEquals(arrayOf("-i", "file one.mp4"), argumentArray)
    }

    @Test
    fun argumentsToString() {
        Assert.assertEquals("null", argumentsToString(null))
        Assert.assertEquals("-i input.mp4 -vf filter -c:v mpeg4 output.mp4", argumentsToString(arrayOf("-i", "input.mp4", "-vf", "filter", "-c:v", "mpeg4", "output.mp4")))
    }

    private fun argumentsToString(arguments: Array<String>?): String {
        return FFmpegKitConfig.argumentsToString(arguments)
    }

    private fun assertMediaInput(mediaInformation: MediaInformation, format: String?, filename: String?) {
        Assert.assertEquals(format, mediaInformation.getFormat())
        Assert.assertEquals(filename, mediaInformation.getFilename())
    }

    private fun assertMediaDuration(mediaInformation: MediaInformation, duration: String?, startTime: String?, bitrate: String?) {
    ...
    private fun assertTag(mediaInformation: MediaInformation, expectedKey: String, expectedValue: String) {
        val tags: JSONObject? = mediaInformation.getTags()
        Assert.assertNotNull(tags)

        try {
            val value = tags!!.getString(expectedKey)
    ...
    private fun assertStreamTag(streamInformation: StreamInformation, expectedKey: String, expectedValue: String) {
        val tags: JSONObject? = streamInformation.getTags()
        Assert.assertNotNull(tags)
    ...
    private fun assertAudioStream(streamInformation: StreamInformation, index: Long?, codec: String?, fullCodec: String?, sampleRate: String?, channelLayout: String?, sampleFormat: String?, bitrate: String?) {
    ...
    private fun assertVideoStream(streamInformation: StreamInformation, index: Long?, codec: String?, fullCodec: String?, format: String?, width: Long?, height: Long?, sar: String?, dar: String?, bitrate: String?, averageFrameRate: String?, realFrameRate: String?, timeBase: String?, codecTimeBase: String?) {
        Assert.assertEquals(index, streamInformation.getIndex())
    ...
    private fun assertChapter(chapter: Chapter, id: Long?, timeBase: String?, start: Long?, startTime: String?, end: Long?, endTime: String?) {
    ...
    companion object {
        private val MEDIA_INFORMATION_MP3 =
            "{\n" +
                    "     \"streams\": [\n" +
                    "         {\n" +
                    "             \"index\": 0,\n" +
        ...
        private val MEDIA_INFORMATION_JPG =
            "{\n" +
                    "     \"streams\": [\n" +
                    "         {\n" +
                    "             \"index\": 0,\n" +
        ...
        private val MEDIA_INFORMATION_GIF =
            "{\n" +
                    "     \"streams\": [\n" +
                    "         {\n" +
                    "             \"index\": 0,\n" +
        ...
        private val MEDIA_INFORMATION_MP4 =
            "{\n" +
                    " \"streams\": [\n" +
                    "      {\n" +
                    "          \"index\": 0,\n" +
        ...
        private val MEDIA_INFORMATION_PNG =
            "{\n" +
                    "     \"streams\": [\n" +
                    "         {\n" +
                    "             \"index\": 0,\n" +
        ...
        private val MEDIA_INFORMATION_OGG =
            "{\n" +
                    "    \"streams\": [\n" +
                    "        {\n" +
                    "            \"index\": 0,\n" +
