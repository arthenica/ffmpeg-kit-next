 ...
 * It is derived from FFmpeg's fftools/ffprobe.c at tag n9.0.1.
 *
 ...
 * - FFmpegKitNext integration updates preserved, including wrapper API,
 *   callbacks, cancellation and thread/session-local execution where applicable
 * - Explicit braces added for -Werror dangling-else builds
 * - Unused unit_second_str constant removed for -Werror builds
 *
 * 06.2026
 * --------------------------------------------------------
...
static __thread int do_analyze_frames = 0;
static __thread int do_bitexact = 0;
static __thread int do_count_frames = 0;
static __thread int do_count_packets = 0;
static __thread int do_read_frames  = 0;
static __thread int do_read_packets = 0;
static __thread int do_show_chapters = 0;
static __thread int do_show_error   = 0;
static __thread int do_show_format  = 0;
static __thread int do_show_frames  = 0;
static __thread int do_show_packets = 0;
static __thread int do_show_programs = 0;
static __thread int do_show_stream_groups = 0;
static __thread int do_show_stream_group_components = 0;
static __thread int do_show_streams = 0;
static __thread int do_show_stream_disposition = 0;
static __thread int do_show_stream_group_disposition = 0;
static __thread int do_show_data    = 0;
static __thread int do_show_program_version  = 0;
static __thread int do_show_library_versions = 0;
static __thread int do_show_pixel_formats = 0;
static __thread int do_show_pixel_format_flags = 0;
static __thread int do_show_pixel_format_components = 0;
static __thread int do_show_log = 0;

static __thread int do_show_chapter_tags = 0;
static __thread int do_show_format_tags = 0;
static __thread int do_show_frame_tags = 0;
static __thread int do_show_program_tags = 0;
static __thread int do_show_stream_group_tags = 0;
static __thread int do_show_stream_tags = 0;
static __thread int do_show_packet_tags = 0;

static __thread int show_value_unit              = 0;
static __thread int use_value_prefix             = 0;
static __thread int use_byte_value_binary_prefix = 0;
static __thread int use_value_sexagesimal_format = 0;
static __thread int show_private_data            = 1;

static __thread const char *audio_codec_name = NULL;
static __thread const char *data_codec_name = NULL;
static __thread const char *subtitle_codec_name = NULL;
static __thread const char *video_codec_name = NULL;

#define SHOW_OPTIONAL_FIELDS_AUTO       -1
#define SHOW_OPTIONAL_FIELDS_NEVER       0
#define SHOW_OPTIONAL_FIELDS_ALWAYS      1
static __thread int show_optional_fields = SHOW_OPTIONAL_FIELDS_AUTO;

static __thread char *output_format;
static __thread char *stream_specifier;
static __thread char *show_data_hash;
static __thread char *data_dump_format;
...
static __thread EntrySelection selected_entries[FF_ARRAY_ELEMS(sections)] = { 0 };

static __thread const OptionDef *options;
static __thread OptionDef *options_local;

/* FFprobe context */
static __thread const char *input_filename;
static __thread const char *print_input_filename;
static __thread const AVInputFormat *iformat = NULL;
static __thread const char *output_filename = NULL;

static const char unit_hertz_str[]          = "Hz"   ;
static const char unit_byte_str[]           = "byte" ;
static const char unit_bit_per_second_str[] = "bit/s";

static __thread unsigned int nb_streams;
static __thread uint64_t *nb_streams_packets;
static __thread uint64_t *nb_streams_frames;
static __thread int *selected_streams;
static __thread int *streams_with_closed_captions;
static __thread int *streams_with_film_grain;
...
static __thread int log_buffer_size;
static __thread int log_print_prefix = 1;
...
/*
 * FFmpegKitNext: capture one log line into ffprobe's -show_log buffer.
 *
 * Replaces upstream's log_callback, which was installed via av_log_set_callback()
 * for -show_log (a process-global hijack that then had to be manually restored via
 * ffmpegkit_restore_log_callback()). This is now a passive duty invoked by the
 * FFmpegKit log callback; it self-guards on do_show_log, so it is a no-op unless
 * -show_log is active. ffprobe runs on a single thread (no scheduler workers), so
 * do_show_log needs no context propagation.
 *
 * The caller passes a private va_copy, so this consumes vl directly.
 * MUST NOT call av_log() (that would recurse back into the log callback).
 */
...
static void show_help_default_ffprobe(const char *opt, const char *arg);

static int show_help_ffprobe(void *optctx, const char *opt, const char *arg)
{
    return show_help_common(optctx, opt, arg, show_help_default_ffprobe);
}
...
static void print_section(SectionID id, int level)
{
    ...
    av_log(NULL, AV_LOG_STDERR, "%c%c%c%c",
...
static void ffprobe_options_uninit(void)
{
    av_freep(&options_local);
    options = NULL;
}

static int ffprobe_options_init(void)
{
    OptionDef init_options[] = {
    CMDUTILS_COMMON_OPTIONS(show_help_ffprobe)
    ...
    ffprobe_options_uninit();
    options_local = av_memdup(init_options, sizeof(init_options));
    if (!options_local)
        return AVERROR(ENOMEM);
...
static void ffprobe_var_cleanup(void)
{
    do_analyze_frames = 0;
    do_bitexact = 0;
    do_count_frames = 0;
    ...
    streams_with_closed_captions = NULL;
    streams_with_film_grain = NULL;
    log_buffer = NULL;
    log_buffer_size = 0;
    log_print_prefix = 1;
...
int ffprobe_execute(int argc, char **argv)
{
    ...
    program_name = ffprobe_program_name;
    program_birth_year = 2007;
    ffprobe_var_cleanup();
    ret = ffprobe_options_init();
    if (ret < 0)
        goto end;
