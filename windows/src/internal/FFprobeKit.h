...
#ifndef FFMPEG_KIT_INTERNAL_FFPROBE_KIT_H
#define FFMPEG_KIT_INTERNAL_FFPROBE_KIT_H
 ...
 *
 * auto asyncSession = FFprobeKit::executeAsync("-hide_banner -v error
 * -show_entries format=size -of default=noprint_wrappers=1 file1.mp4", [](auto
 * session){ ... });
 * </pre>
 * <p>Provides overloaded <code>execute</code> methods to define session
 * specific callbacks. <pre> auto session =
 * FFprobeKit::executeAsync("-hide_banner -v error -show_entries format=size -of
 * default=noprint_wrappers=1 file1.mp4", [](auto session){ ... }, [](auto log){
 * ... }];
 * </pre>
 * <p>It can extract media information for a file or a url, using
 * getMediaInformation method. <pre> auto session =
...
class FFprobeKit {
  public:
     ...
     */
    static std::shared_ptr<ffmpegkit::internal::FFprobeSession>
    ...
    /**
     * <p>Synchronously executes FFprobe command provided. Space character is
     ...
     */
    static std::shared_ptr<ffmpegkit::internal::MediaInformationSession>
    ...
    /**
     * <p>Starts an asynchronous FFprobe execution to extract the media
...
#endif // FFMPEG_KIT_INTERNAL_FFPROBE_KIT_H
