...
#ifndef FFMPEG_KIT_INTERNAL_FFMPEG_KIT_SESSION_H
#define FFMPEG_KIT_INTERNAL_FFMPEG_KIT_SESSION_H
...
class Session {
  public:
     ...
     */
    virtual ffmpegkit::internal::LogCallback getLogCallback() const = 0;
     ...
     */
    virtual std::shared_ptr<std::list<std::shared_ptr<ffmpegkit::internal::Log>>>
    getAllLogsWithTimeout(const int waitTimeout) const = 0;
     ...
     * are asynchronous messages that are not delivered yet, this method will
     * not wait for them and will return immediately.
     *
     ...
     * string. If there are asynchronous messages that are not delivered yet,
     * this method waits for them until the given timeout.
     *
     * @param waitTimeout wait timeout for asynchronous messages in milliseconds
     * @return all log entries generated for this session as a concatenated
     * string
     */
    virtual std::string
    getAllLogsAsStringWithTimeout(const int waitTimeout) const = 0;
     ...
     * string. Note that if there are asynchronous messages that are not
     ...
     */
    virtual ffmpegkit::internal::SessionState getState() const = 0;

    /**
     * Returns the return code for this session. Note that return code is only
     * set for sessions that end with SessionStateCompleted state. If a session
     * is not started, still running or failed then this method returns nullptr.
     *
     ...
     */
    virtual std::shared_ptr<ffmpegkit::internal::ReturnCode> getReturnCode() const = 0;
     ...
     * SessionStateFailed state. For sessions that has SessionStateCompleted
     * state this method returns an empty string.
     *
     * @return stack trace of the exception received while executing this
     * session, an empty string if session is not started, still running or
     * completed
     */
     ...
     * @param log log entry
     */
    virtual void addLog(const std::shared_ptr<ffmpegkit::internal::Log> log) = 0;
     ...
     */
    virtual void
    complete(const std::shared_ptr<ffmpegkit::internal::ReturnCode> returnCode) = 0;
...
#endif // FFMPEG_KIT_INTERNAL_FFMPEG_KIT_SESSION_H
