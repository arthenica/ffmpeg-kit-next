...
#ifndef FFMPEG_KIT_INTERNAL_FFMPEG_KIT_CONFIG_H
#define FFMPEG_KIT_INTERNAL_FFMPEG_KIT_CONFIG_H
...
class FFmpegKitConfig {
  public:
     ...
     */
    static void ignoreSignal(const ffmpegkit::internal::Signal signal);
     ...
     * @param waitTimeout             max time to wait until media information
     * is transmitted
     */
     ...
     * execution to complete. You must use an FFmpegSessionCompleteCallback if
    ...
    /**
     * <p>Starts an asynchronous FFprobe execution for the given media
     * information session.
     *
     * <p>Note that this method returns immediately and does not wait the
     ...
     * defined log callback
     */
    static void enableLogCallback(const ffmpegkit::internal::LogCallback logCallback);
    ...
    static void enableStatisticsCallback(
        const ffmpegkit::internal::StatisticsCallback statisticsCallback);

    /**
     * <p>Sets a global FFmpegSessionCompleteCallback to receive execution
     * results for FFmpeg sessions.
     *
     ...
     *
     * @return global MediaInformationSessionCompleteCallback or nullptr if it
     * is not set
     */
     ...
     */
    static ffmpegkit::internal::Level getLogLevel();
     ...
     */
    static void setLogLevel(const ffmpegkit::internal::Level level);
     ...
     * @return string value
     */
    static std::string logLevelToString(const ffmpegkit::internal::Level level);
     ...
     *
     * @param sessionHistorySize session history size, should be smaller than
     * 1000
     */
    static void setSessionHistorySize(const int sessionHistorySize);

    /**
     * Returns the session specified with <code>sessionId</code> from the
     * session history.
     *
     ...
     * the history
     */
    static std::shared_ptr<ffmpegkit::internal::Session> getSession(const long sessionId);
     ...
     */
    static std::shared_ptr<ffmpegkit::internal::Session> getLastSession();
     ...
     */
    static std::shared_ptr<ffmpegkit::internal::Session> getLastCompletedSession();
     ...
     */
    static std::shared_ptr<std::list<std::shared_ptr<ffmpegkit::internal::Session>>>
    getSessions();
     ...
     */
    static std::shared_ptr<std::list<std::shared_ptr<ffmpegkit::internal::FFmpegSession>>>
    getFFmpegSessions();
     ...
     * @return number of async messages that are not transmitted to the
     * callbacks for this session
     */
...
#endif // FFMPEG_KIT_INTERNAL_FFMPEG_KIT_CONFIG_H
