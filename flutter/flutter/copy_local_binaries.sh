...
# "prebuilt/" directory into this Flutter plugin's platform folders so the
...
# Plugin lives at <repo>/flutter/flutter, so prebuilt is two levels up.
...
copy_apple() {
  ...
  dest="${plugin_dir}/${dest_subdir}/ffmpeg_kit_next_flutter/Frameworks"
  rm -rf "${dest}"
  mkdir -p "${dest}"
  local count=0
